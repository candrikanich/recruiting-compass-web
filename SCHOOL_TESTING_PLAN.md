...
it('should calculate correct match score with weighted preferences', () => {
  const school = createMockSchool({
    ...
    conference: 'SEC',
    location: 'Florida',
    academic_info: { admission_rate: 0.20 },
  })
  ...
  const preferences = [
    ...
    { type: 'conference_type', value: ['Power 4'], priority: 3, is_dealbreaker: false }, // weight=8, match
  ]
...
])('should render $size variant with correct dimensions', async ({ size, pixels, font }) => {
...
it('should calculate correct fit score from components', async () => {
  ...
  mockSupabase.from('schools').select().eq().single()
    .mockResolvedValue({ data: { id: 'school-1' }, error: null })

  // Mock fit score calculation: 75/100
  calculateFitScore.mockReturnValue({
    score: 75,
    breakdown: { athleticFit: 30, academicFit: 20, opportunityFit: 15, personalFit: 10 }
  })
