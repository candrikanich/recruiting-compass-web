...
export const exportInteractionsToCSV = (
  ...
  downloadFile(csv, filename || `interactions-${date}.csv`, 'text/csv;charset=utf-8;')
}
...
export const exportSchoolComparisonToCSV = (
  ...
  const rows = schools.map(s => [
    ...
    s.offer?.deadline_date ? new Date(s.offer.deadline_date).toLocaleDateString() : '',
    (s.pros || []).join('; '),
    (s.cons || []).join('; '),
  ])
  ...
  const csv = toCSV(headers, rows)
  const date = new Date().toISOString().split('T')[0]
  downloadFile(csv, filename || `school-comparison-${date}.csv`, 'text/csv;charset=utf-8;')
}
...
export const generateInteractionsPDF = (
  ...
  const positiveCount = interactions.filter(i => i.sentiment === 'positive' || i.sentiment === 'very_positive').length
...
export const generateSchoolComparisonPDF = (
  ...
  const avgDistance = schools.filter(s => s.distance).length > 0
    ? Math.round(schools.filter(s => s.distance).reduce((sum, s) => sum + (s.distance || 0), 0) / schools.filter(s => s.distance).length)
    : 0
 ...
 */
export const chartToImage = async (chartElement: HTMLCanvasElement): Promise<string> => {
  if (!chartElement) {
...
export const exportAnalyticsPDF = async (
  ...
  try {
    ...
    doc.text(`Period: ${dateRange?.start || 'N/A'} to ${dateRange?.end || 'N/A'}`, margin, margin + 25)
    ...
    if (summaryStats.length > 0) {
      ...
      const tableData = summaryStats.map(stat => [stat.label, String(stat.value)])
      doc.autoTable({
        head: [['Metric', 'Value']],
        body: tableData,
        startY: yOffset,
