...
export const FIT_WEIGHTS = {
  athletic: 0.40, // 40 points max
  academic: 0.25, // 25 points max
  opportunity: 0.20, // 20 points max
...
export function calculateFitScore(inputs: Partial<FitScoreInputs>): FitScoreResult {
  ...
  normalized.athleticFit = Math.max(0, Math.min(40, normalized.athleticFit ?? 0))
  normalized.academicFit = Math.max(0, Math.min(25, normalized.academicFit ?? 0))
  normalized.opportunityFit = Math.max(0, Math.min(20, normalized.opportunityFit ?? 0))
  normalized.personalFit = Math.max(0, Math.min(15, normalized.personalFit ?? 0))
...
export function calculateAthleticFit(
  ...
  if (athletePosition && schoolPositionNeeds.length > 0) {
    if (schoolPositionNeeds.includes(athletePosition)) {
      score += 10
    } else if (schoolPositionNeeds.some(p => p.includes('OF') && athletePosition.includes('OF'))) {
      score += 7
    } else if (schoolPositionNeeds.some(p => p.includes('IF') && athletePosition.includes('IF'))) {
      score += 7
    } else {
      score += 3
    }
  ...
  if (athleteHeight && athleteWeight) {
    ...
    if ((athleteHeight >= 69 && athleteHeight <= 76) && (athleteWeight >= 180 && athleteWeight <= 220)) {
      score += 8
    } else if ((athleteHeight >= 67 && athleteHeight <= 78) && (athleteWeight >= 160 && athleteWeight <= 240)) {
      score += 5
    } else {
      score += 2
    }
...
export function calculateAcademicFit(
  ...
  if (targetMajor && schoolOfferedMajors.length > 0) {
    if (schoolOfferedMajors.some(m => m.toLowerCase().includes(targetMajor.toLowerCase()))) {
      score += 5
    } else {
      score += 2
    }
...
export function calculatePersonalFit(
  ...
  // Campus size fit (0-3)
  const campusSizeScore = getCampusSizeScore(athleteCampusSizePreference, schoolCampusSize)
 ...
 */
function getCampusSizeScore(preference: 'small' | 'medium' | 'large', population: number): number {
...
export function calculatePortfolioHealth(
  ...
  let status: 'healthy' | 'needs_attention' | 'at_risk' | 'not_started' = 'healthy'
 ...
 */
export function getFitScoreRecommendation(score: number, tier: FitTier): string {
  switch (tier) {
    case 'match':
