...
class PerformanceMonitor {
  ...
  end(label: string, category: 'api' | 'component' | 'operation' = 'operation'): number {
  ...
  measure(label: string, fn: () => void, category?: 'api' | 'component' | 'operation'): number {
    this.start(label)
    fn()
  ...
  getMetrics(category?: 'api' | 'component' | 'operation'): PerformanceMetric[] {
    return category ? this.metrics.filter(m => m.category === category) : this.metrics
  }
  ...
  getStats(): {
    ...
    min: number
    max: number
    byCategory: Record<string, { count: number; average: number; total: number }>
  } {
    ...
    const byCategory: Record<string, { count: number; average: number; total: number }> = {}
