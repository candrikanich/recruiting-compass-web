...
export const generateReportData = (
  ...
  filteredInteractions.forEach(i => {
    ...
    if (i.sentiment) {
      interactionsBySentiment[i.sentiment] = (interactionsBySentiment[i.sentiment] || 0) + 1
    }
  })
  ...
  const avgResponseRate = coachResponseRates.length > 0
    ? coachResponseRates.reduce((a, b) => a + b, 0) / coachResponseRates.length
    : 0
...
export const exportReportToCSV = (report: ReportData): string => {
  ...
  lines.push('"Average Response Rate",' + (report.coaches?.avgResponseRate || 0))
  lines.push('')
  ...
  if (report.metrics?.summaries) {
    ...
    report.metrics.summaries.forEach(m => {
      lines.push(',"","' + m.type + '",' + m.avg.toFixed(2) + ',' + m.max + ',' + m.min)
    })
  }
