...
export const generateCoachEmailTemplate = (
  ...
  metrics.forEach((m) => {
    if (
      !latestByType[m.metric_type] ||
      new Date(m.recorded_date) >
        new Date(latestByType[m.metric_type].recorded_date)
    ) {
  ...
  const highlights = Object.values(latestByType)
    .map(
      (m) =>
        `- ${m.metric_type.replace(/_/g, " ").toUpperCase()}: ${m.value} ${m.unit}`,
    )
    .join("\n");
