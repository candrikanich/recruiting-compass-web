...
export const sanitizeSchoolFields = <T extends SchoolFields>(data: T): T => ({
  ...data,
  ...(data.notes != null && { notes: sanitizeHtml(data.notes) }),
  ...(data.pros != null && { pros: data.pros.filter((p): p is string => p != null && p !== "").map((p) => sanitizeHtml(p)) }),
  ...(data.cons != null && { cons: data.cons.filter((c): c is string => c != null && c !== "").map((c) => sanitizeHtml(c)) }),
