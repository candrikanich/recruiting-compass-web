import { initializePDF, addHeader, addFooter, addMetricsTable, addChartImage } from './pdfHelpers'
import { generateCoachEmailTemplate, generateEventSummaryTemplate, getMetricLabel } from './textTemplates'
...
export const generateIndividualMetricReport = (
  ...
  if (format === 'pdf') {
    ...
    // Text format
    return generateCoachEmailTemplate(athleteName, options.coachName || 'Coach', filteredMetrics)
  }
}
...
export const generateComprehensiveReport = (options: ReportOptions): string | Blob => {
  ...
  if (format === 'pdf') {
    ...
    return generateCoachEmailTemplate(athleteName, options.coachName || 'Coach', metrics)
  }
}
...
export const generateEventReport = (options: ReportOptions): string | Blob => {
  ...
  if (format === 'pdf') {
    ...
    doc.setFontSize(12)
    doc.setFont('helvetica', 'normal')
    doc.text(`Date: ${new Date(event.start_date).toLocaleDateString()}`, 15, yPos)
