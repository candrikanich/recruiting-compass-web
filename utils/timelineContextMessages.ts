  ...
  switch (context) {
    case "schools":
      return {
        ...
        sophomore: () =>
          "Build a target list of 20-25 schools across all fit tiers",
        junior: (data) =>
          data.needsMoreSafeties
            ? "Add 2-3 safety schools to ensure your options"
            : "Your portfolio looks balanced",
        senior: (data) =>
          `Focus outreach on your top ${data.schoolCount || 5} schools with strongest interest`,
        committed: () =>
          "Process complete — congratulations on your commitment!",
      };

    case "coaches":
      return {
        freshman: () =>
          "Focus on skill development — direct coach contact comes later",
        sophomore: (data) =>
          `${data.coachesToContact || 3} coaches are ready for initial contact`,
        junior: (data) =>
          `${data.staleCoaches || 2} coaches need follow-up this week`,
        senior: (data) =>
          `Stay in touch with ${data.activeCoaches || 3} coaches with strong interest`,
        committed: () =>
          "Maintain regular contact with your future coaching staff",
      };
    ...
    case "interactions":
      return {
        ...
        sophomore: (data) =>
          `Log your recent ${data.eventType || "event"} attendance`,
        junior: (data) =>
          `${data.pendingFollowUps || 2} interactions need follow-up`,
        senior: (data) =>
          `${data.recentInteractions || 3} interactions this month — keep it up!`,
        committed: () =>
          "Continue building relationships with your future team",
      };

    case "events":
      return {
        freshman: (data) =>
          `${data.upcomingEvents || 2} showcases coming up this semester`,
        sophomore: (data) =>
          `${data.needsRSVP || 3} events need RSVP confirmation`,
        junior: (data) =>
          `Prioritize these ${data.highValueEvents || 3} high-value events`,
        senior: (data) =>
          `${data.finalEvents || 2} commitment events scheduled`,
