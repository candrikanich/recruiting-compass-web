...
export function getParentActionItems(context: ParentMessageContext): string[] {
  ...
  if (!context.division) {
    actions.push('Help your athlete identify target divisions (D1, D2, D3, NAIA, JUCO)')
  }

  if (!context.schoolCount || context.schoolCount < 10) {
    actions.push('Build a list of 15-20 target schools at appropriate competitive levels')
  }
  ...
  if (context.phase === 'freshman' || context.phase === 'sophomore') {
    actions.push('Focus on training and academics now—recruiting will intensify later')
  }
  ...
  if (context.phase === 'senior' && context.status === 'at_risk') {
    actions.push('Expand target school list to include D2, D3, NAIA, and JUCO options')
  }
 ...
 */
export function getRecruitingExpectations(phase: Phase, division?: Division): string {
  const expectations: Record<Phase, Record<string, string>> = {
    freshman: {
      ...
      default: 'Freshman year is about building skills. Most recruiting starts in sophomore/junior year.',
    },
    sophomore: {
      ...
      default: 'Sophomore year is when coaches start noticing. Increase visibility.',
    },
    junior: {
    ...
    senior: {
      ...
      default: 'Senior year is final decision year. Opportunities still exist at all levels.',
    },
    committed: {
      ...
      default: 'Your athlete has committed. Support their preparation for college.',
    },
  }

  const divisionKey = (division || 'default') as keyof typeof expectations[Phase]
