 ...
 */
export const addHeader = (doc: jsPDF, title: string, subtitle?: string): void => {
  doc.setFontSize(20)
...
export const addFooter = (doc: jsPDF, pageNumber: number): void => {
  ...
  doc.text(`Generated: ${new Date().toLocaleDateString()}`, 190, pageHeight - 10, { align: 'right' })
}

/**
 * Add metrics table to PDF
 */
export const addMetricsTable = (doc: jsPDF, metrics: PerformanceMetric[], yPosition: number): number => {
 ...
 */
export const addChartImage = (doc: jsPDF, base64Image: string, title: string, yPosition: number): number => {
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
