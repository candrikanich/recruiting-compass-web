...
Deno.serve(async () => {
  ...
  for (const user of users ?? []) {
    ...
    for (const o of offers ?? []) {
      const schoolName =
        (o.schools as { name: string } | null)?.name ?? "Unknown School";
      deadlines.push({
        source_table: "offers",
        source_id: o.id,
    ...
    const gradYear = (
      userDetail?.player_details as { graduation_year?: number } | null
    )?.graduation_year;
    const sport = (
      userDetail?.player_details as { primary_sport?: string } | null
    )?.primary_sport;
    ...
    if (gradYear) {
      ...
      for (const s of sc ?? []) {
        deadlines.push({
          source_table: "system_calendar",
          source_id: s.id,
          deadline_date: s.start_date,
          label: s.label,
          priority: "normal",
        });
      }
