  ...
  for (const user of users ?? []) {
    ...
    const lines: string[] = hasActivity
      ? [
          ...
          ...(schoolChanges > 0
            ? [
                `${schoolChanges} school status change${schoolChanges !== 1 ? "s" : ""}`,
              ]
            : []),
          ...((upcoming?.length ?? 0) > 0
            ? [
    ...
    if (insertError) {
      console.error(
        `Failed to insert notification for user ${user.id}:`,
        insertError,
      );
      continue;
    }
    ...
    if (user.email && emailEnabled) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Failed to send digest email for user ${user.id}: HTTP ${response.status} - ${errorText}`,
          );
