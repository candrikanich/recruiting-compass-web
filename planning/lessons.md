...
- **Symptom**: a spec passes alone + in local full run, fails deterministically in CI. `TypeError: Cannot read properties of undefined (reading 'query')`.
- **Root cause**: `pages/interactions/add.vue` reads `useRoute().query.coachId`; the spec never mocked `vue-router`'s `useRoute` (global setup mocks `#app` `useRouter`, NOT `vue-router` `useRoute`). The page free-rode on a router context **leaked from whatever spec ran before it**. CI's Linux file-order put the provider AFTER it → `useRoute()` undefined. macOS file-order is lucky → local green.
- **Diagnostic that mattered**: use **file-only** shuffle to reproduce a CI order bug — `--sequence.shuffle.files=true --sequence.shuffle.tests=false`. Plain `--sequence.shuffle` shuffles tests WITHIN files too and floods you with intra-file order failures (27-51) that are NOT the CI cause (CI never reorders tests within a file). File-only shuffle reproduced EXACTLY the CI failure (only interactions-add, every seed).
- **Trap avoided**: the intra-file shuffle noise made it look like a suite-wide isolation crisis needing a 550-file refactor. A global `vi.resetAllMocks()` in setup's beforeEach REGRESSED 31 tests (it wipes the idiomatic module-scope `vi.mock` factory defaults like `requireAdmin: vi.fn().mockResolvedValue(...)`). The real fix was ONE line: mock `useRoute` in the one fragile spec.
- **Also disproven**: pinning vitest 4→3 made shuffle WORSE (39-51 vs 3-27), so the vitest major was not the cause — the fragility is version-independent.
- **Rule**: any page/composable that calls `useRoute`/`useRouter` from `vue-router` must have it mocked in its spec; don't rely on another spec providing router context. Diagnose CI-only order bugs with file-only shuffle, not full shuffle.

## Run the FULL unit suite after endpoint changes — type-check is not enough (2026-08-27)
Inbound-lead-to-interaction: per-task type-checks were green but existing endpoint tests (`tests/unit/server/api/public/{contact,interest}.spec.ts`) went red for 3 tasks and I only caught it at the final gate. Also dispatched a task claiming "no endpoint harness exists" — wrong, it did.
**Rules:** (1) after any `server/api/**` change, run `npm run test` (full unit), not just `type-check`. (2) Before calling a test file "(new)" in a plan, `git cat-file -e HEAD:<path>` — pre-existing test files must be EXTENDED, never overwritten (an implementer clobbered 5 security tests down to 2 this session). (3) When an endpoint skips a notification expecting a DB trigger to cover it, check the trigger's WHERE clause — `notify_on_inbound_interaction` excludes `logged_by`, so a player-logged interaction never notifies the player.
