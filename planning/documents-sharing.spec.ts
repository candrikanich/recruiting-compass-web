...
test.describe('Document Sharing Workflows', () => {
  ...
  test('should share document with another user', async ({ page }) => {
    ...
    // Click first result
    const firstResult = page.locator('[data-testid="user-search-result"]').first()
  ...
  test('should share document with view permission', async ({ page }) => {
    ...
    const permissionDesc = page.locator('[data-testid="permission-description"]')
  ...
  test('should not share with invalid email', async ({ page }) => {
    ...
    const hasError = (await noResults.isVisible()) || (await errorMsg.isVisible())
  ...
  test('should update sharing permission from view to edit', async ({ page }) => {
    // Navigate to document
    await page.locator('[data-testid="document-card"]').first().click()
    await page.waitForLoadState('networkidle')
    ...
    const sharedList = page.locator('[data-testid="shared-users-list"]')
    const sharedUsers = await sharedList.locator('[data-testid="shared-user-row"]').count()
  ...
  test('should update sharing permission from edit to view', async ({ page }) => {
    // Navigate to document
    await page.locator('[data-testid="document-card"]').first().click()
    await page.waitForLoadState('networkidle')

    // Verify document is shared with edit access
    const firstUserRow = page.locator('[data-testid="shared-user-row"]').first()
  ...
  test('should add expiration date to shared access', async ({ page }) => {
    ...
    // Enable expiration
    const expirationCheckbox = page.locator('[data-testid="enable-expiration"]')
  ...
  test('should revoke document access from user', async ({ page }) => {
    ...
    const sharedList = page.locator('[data-testid="shared-users-list"]')
    const initialCount = await sharedList.locator('[data-testid="shared-user-row"]').count()
    ...
    if (initialCount > 0) {
      ...
      if (await confirmBtn.isVisible()) {
        ...
        const newCount = await sharedList.locator('[data-testid="shared-user-row"]').count()
  ...
  test('should revoke all access to document', async ({ page }) => {
    ...
    if (await removeAllBtn.isVisible()) {
      ...
      const sharedList = page.locator('[data-testid="shared-users-list"]')
      const userCount = await sharedList.locator('[data-testid="shared-user-row"]').count()
  ...
  test('should view shared document with view-only access', async ({ page }) => {
  ...
  test('should set document to private (no sharing)', async ({ page }) => {
    ...
    if (await settingsBtn.isVisible()) {
      ...
      // Find privacy option
      const privateOption = page.locator('[data-testid="privacy-option-private"]')
      if (await privateOption.isVisible()) {
        await privateOption.click()

        // Confirm
        const confirmBtn = page.locator('[data-testid="confirm-privacy-change"]')
        if (await confirmBtn.isVisible()) {
          await confirmBtn.click()
  ...
  test('should set document to shareable', async ({ page }) => {
    ...
    if (await settingsBtn.isVisible()) {
      ...
      const shareableOption = page.locator('[data-testid="privacy-option-shareable"]')
