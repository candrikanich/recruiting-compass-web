...
test.describe('Search & Filter Workflows', () => {
  ...
  test('should clear search and show all documents', async ({ page }) => {
    ...
    const allCount = await page.locator('[data-testid="document-card"]').count()
  ...
  test('should show no results message for non-matching search', async ({ page }) => {
    ...
    const hasNoResults = (await noResults.isVisible()) || (await emptyState.isVisible())
  ...
  test('should filter by multiple document types', async ({ page }) => {
    ...
    const typeCheckboxes = page.locator('[data-testid="type-option-highlight_video"]')
    if (await typeCheckboxes.isVisible()) {
      await typeCheckboxes.check()
    }

    const transcriptCheckbox = page.locator('[data-testid="type-option-transcript"]')
  ...
  test('should remove individual filters', async ({ page }) => {
    ...
    // Apply a filter
    const typeCheckbox = page.locator('[data-testid="type-option-highlight_video"]')
  ...
  test('should clear all filters', async ({ page }) => {
    ...
    if (await clearAllBtn.isVisible()) {
      ...
      const checkedBoxes = page.locator('[data-testid="type-option-highlight_video"]:checked')
  ...
  test('should sort results by date (newest first)', async ({ page }) => {
    ...
    if (await sortDropdown.isVisible()) {
      ...
      if (count >= 2) {
        ...
        const firstDate = await results.nth(0).locator('[data-testid="document-date"]').textContent()
        const secondDate = await results.nth(1).locator('[data-testid="document-date"]').textContent()
  ...
  test('should search with multiple criteria combined', async ({ page }) => {
    ...
    // Apply type filter
    const videoFilter = page.locator('[data-testid="type-option-highlight_video"]')
