...
test.describe("Document CRUD Workflows", () => {
  ...
  test("should view document details page", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="document-created-date"]'),
    ).toBeVisible();
  ...
  test("should display document metadata and info", async ({ page }) => {
    ...
    await expect(typeField).toHaveText(
      /highlight_video|transcript|resume|rec_letter|stats_sheet/,
    );
  ...
  test("should restore document to previous version", async ({ page }) => {
    ...
    if (await versionTab.isVisible()) {
      ...
      const restoreBtn = page
        .locator('[data-testid="restore-version-btn"]')
        .first();
  ...
  test("should complete document workflow from upload to sharing", async ({
    page,
  }) => {
    // 1. Upload document
    await page.click('[data-testid="upload-document-btn"]');
    ...
    await page.click('[data-testid="save-document-btn"]');
    await expect(
      page.locator("text=Document updated successfully"),
    ).toBeVisible();
