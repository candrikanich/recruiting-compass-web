...
export const schoolsFixture = {
  ...
  async createCustomSchool(page: Page, schoolData: {
    name: string
    state: string
    division: string
  }) {
    ...
    await page.selectOption('[data-testid="school-division"]', schoolData.division)
  ...
  async findSchoolInList(page: Page, schoolName: string) {
    const schoolCard = page.locator(`[data-testid="school-card"][data-name="${schoolName}"]`)
    return schoolCard
  },
...
test.describe("Story 3.1: Add Schools to Track List", () => {
  ...
  test.describe("Scenario: Add school from database", () => {
    ...
    test("should auto-populate school information from database", async ({ page }) => {
      await schoolsFixture.createSchoolFromDatabase(page, "Arizona State University")
      ...
      await expect(page.locator('[data-testid="school-division"]')).toContainText("D1")
      await expect(page.locator('[data-testid="school-location"]')).not.toHaveValue("")
      await expect(page.locator('[data-testid="school-conference"]')).not.toHaveValue("")
    })
  ...
  test.describe("Scenario: Add custom school", () => {
    test("should add school manually when not in database", async ({ page }) => {
  ...
  test.describe("Scenario: Prevent duplicate schools", () => {
    test("should warn when adding duplicate school", async ({ page }) => {
      ...
      const warningModal = page.locator('[data-testid="duplicate-warning-modal"]')
    ...
    test("should allow canceling duplicate addition", async ({ page }) => {
      ...
      const cancelButton = page.locator('[data-testid="duplicate-warning-cancel"]')
    ...
    test("should allow proceeding with duplicate if user chooses", async ({ page }) => {
      // Create first school
      await schoolsFixture.createSchoolFromDatabase(page, "Arizona State University")
      ...
      // Click "Add Anyway"
      const addAnywayButton = page.locator('[data-testid="duplicate-warning-add-anyway"]')
  ...
  test.describe("Scenario: Warning at 30+ schools", () => {
    test("should show warning banner when reaching 30 schools", async ({ page }) => {
      ...
      if (schoolCount >= 30) {
        const warningBanner = page.locator('[data-testid="schools-warning-banner"]')
...
test.describe("Story 3.2: View School List with Fit Scores", () => {
  ...
  test.describe("Scenario: Fit score displays for each school", () => {
    ...
    test("should display fit score on school detail page", async ({ page }) => {
      // Click into a school
      const schoolCard = page.locator('[data-testid="school-card"]').first()
      const schoolName = await schoolCard.locator('[data-testid="school-name"]').textContent()
  ...
  test.describe("Scenario: Fit score breakdown is visible", () => {
    test("should show fit score breakdown on demand", async ({ page }) => {
      ...
      // Verify modal opens
      const breakdownModal = page.locator('[data-testid="fit-score-breakdown-modal"]')
  ...
  test.describe("Scenario: Fit score updates automatically", () => {
    test("should recalculate fit score when athlete profile changes", async ({ page }) => {
    ...
    test("should show notification when fit scores update", async ({ page }) => {
  ...
  test.describe("Scenario: Honest assessment - no false hope", () => {
    test("should show low scores for athletes not matching D1 requirements", async ({ page }) => {
...
test.describe("Story 3.3: Filter and Sort Schools", () => {
  ...
  test.describe("Scenario: Filter by priority tier", () => {
    test("should filter schools by priority A", async ({ page }) => {
      ...
      const priorityButton = firstCard.locator('[data-testid="priority-button"]')
      ...
      for (let i = 0; i < count; i++) {
        const tierBadge = cards.nth(i).locator('[data-testid="priority-tier-badge"]')
  ...
  test.describe("Scenario: Filter by fit score range", () => {
    test("should filter schools by fit score 70-100", async ({ page }) => {
      ...
      for (let i = 0; i < count; i++) {
        const scoreText = await cards.nth(i).locator('[data-testid="fit-score"]').textContent()
        const score = parseInt(scoreText?.match(/\d+/)?.[0] || "0")
  ...
  test.describe("Scenario: Sort schools by distance", () => {
    test("should sort schools by distance ascending (closest first)", async ({ page }) => {
      ...
      if (count > 1) {
        const firstDist = parseInt(await distances.first().textContent() || "0")
        const lastDist = parseInt(await distances.last().textContent() || "999999")

        expect(firstDist).toBeLessThanOrEqual(lastDist)
      }
    })

    test("should sort schools by distance descending (farthest first)", async ({ page }) => {
      const sortDropdown = page.locator('[data-testid="sort-dropdown"]')
      await sortDropdown.click()
      await page.locator('[data-testid="sort-distance"]').click()
      ...
      if (count > 1) {
        const firstDist = parseInt(await distances.first().textContent() || "0")
        const lastDist = parseInt(await distances.last().textContent() || "0")
  ...
  test.describe("Scenario: Sort schools by fit score", () => {
    ...
    test("should sort by fit score lowest first", async ({ page }) => {
      ...
      if (count > 1) {
        const firstScore = parseInt(await scores.first().textContent() || "100")
        const lastScore = parseInt(await scores.last().textContent() || "0")
  ...
  test.describe("Scenario: Apply multiple filters together", () => {
    test("should apply priority + fit score filters", async ({ page }) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        // Check fit score
        const score = await card.locator('[data-testid="fit-score"]').textContent()
        const scoreNum = parseInt(score?.match(/\d+/)?.[0] || "0")
        expect(scoreNum).toBeGreaterThanOrEqual(70)
      }
    })

    test("should combine priority + division + distance filters", async ({ page }) => {
...
test.describe("Story 3.4: Set School Priority and Status", () => {
  ...
  test.describe("Scenario: Set school priority tier", () => {
    test("should set school to priority A", async ({ page }) => {
      ...
      const prioritySelector = page.locator('[data-testid="priority-selector"]')
      ...
      await page.goto("/schools")
      const cardBadge = page.locator('[data-testid="school-card"]').first().locator('[data-testid="priority-badge"]')
  ...
  test.describe("Scenario: Priority and status are independent", () => {
    test("should set priority without changing status", async ({ page }) => {
      ...
      const statusBefore = await page.locator('[data-testid="status-badge"]').textContent()
      ...
      const statusAfter = await page.locator('[data-testid="status-badge"]').textContent()
      expect(statusAfter).toBe(statusBefore)
      expect(statusAfter).toContain("interested")
    })

    test("should change status without affecting priority", async ({ page }) => {
      await schoolsFixture.navigateToSchoolsPage(page)
      const firstCard = page.locator('[data-testid="school-card"]').first()
      await firstCard.click()
      await page.waitForLoadState("networkidle")
      ...
      const priorityBefore = await page.locator('[data-testid="priority-badge"]').textContent()
      ...
      const priorityAfter = await page.locator('[data-testid="priority-badge"]').textContent()
...
test.describe("Story 3.5: View and Add School Notes", () => {
  ...
  test.describe("Scenario: Add notes about school", () => {
    test("should add notes to school", async ({ page }) => {
      ...
      await notesTextarea.fill("Great campus, strong athletic program, responsive coaches. Top choice.")
