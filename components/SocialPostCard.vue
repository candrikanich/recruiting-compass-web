<template>
  <div
    ...
    :class="[
      post.flagged_for_review ? 'bg-orange-50 border-l-orange-400' : 'bg-white border-l-slate-300',
    ]"
  >
    <!-- Header -->
    <div class="flex items-start justify-between mb-4">
      <div class="flex-1">
        <div class="flex items-center gap-2 mb-2">
          <ShareIcon v-if="post.platform === 'twitter'" class="w-6 h-6 text-blue-600" />
    ...
    <div class="flex gap-2 mb-3 flex-wrap">
      <span v-if="post.is_recruiting_related" class="inline-block px-2 py-1 text-xs font-semibold rounded bg-emerald-100 text-emerald-800">
        🎯 Recruiting
      </span>
      <span v-if="post.sentiment" class="inline-block px-2 py-1 text-xs font-semibold rounded" :class="getSentimentClass()">
    ...
    <!-- Post Content -->
    <div class="mb-4">
      <p class="whitespace-pre-wrap text-sm line-clamp-6 text-slate-900">{{ post.post_content }}</p>
    </div>

    <!-- Metadata -->
    <div class="flex items-center justify-between pt-4 text-xs border-t border-slate-300 text-slate-600">
      <span>{{ formatDate(post.post_date) }}</span>
      <div class="flex gap-2">
        <span v-if="schoolName" class="text-slate-900 font-medium">{{ schoolName }}</span>
        <NuxtLink
        ...
        <span v-else-if="coachName" class="text-blue-600 font-medium">{{ coachName }}</span>
      </div>
    </div>
    ...
    <!-- Notes Display -->
    <div v-if="post.notes && !showNotes" class="mt-4 pt-4 border-t border-slate-300">
...
import { getSentimentEmoji, getSentimentColor, getSentimentLabel } from '~/utils/sentimentAnalysis'
...
const sentimentColorClass = computed(() => getSentimentColor(props.post.sentiment))
