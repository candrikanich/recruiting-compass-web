<template>
  <div class="rounded-xl border border-slate-200 shadow-sm p-6 bg-white">
    ...
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-lg font-semibold text-slate-900">💬 Quick Communication</h2>
      <button
    ...
    <div v-if="coach" class="mb-6 p-4 rounded-lg border border-blue-200 bg-blue-50">
      <p class="text-sm mb-1 text-slate-900">
        <span class="font-semibold">{{ coach.first_name }} {{ coach.last_name }}</span>
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div class="text-left">
              ...
              <p class="text-sm text-slate-500">{{ formatPhone(coach.phone) }}</p>
            </div>
          </div>
          <span class="text-slate-400">→</span>
        </button>
    ...
    <div v-if="showEmailComposer" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-200">
        <div class="p-6 border-b border-slate-200">
          <h3 class="text-lg font-semibold text-slate-900">Send Email to {{ coach.first_name }}</h3>
        </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <div class="rounded-lg p-3 bg-blue-50 border border-blue-200">
            <p class="text-xs font-semibold mb-2 text-blue-900">Available Variables:</p>
    ...
    <!-- Text Composer Modal -->
    <div v-if="showTextComposer" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-200">
        <div class="p-6 border-b border-slate-200">
          <h3 class="text-lg font-semibold text-slate-900">Send Text to {{ coach.first_name }}</h3>
        </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-xs text-slate-500">{{ textComposer.body.length }}/160</span>
            </div>
            <textarea
              ...
              placeholder="Your message..."
            />
            <p class="text-xs text-slate-500 mt-2">SMS limited to 160 characters</p>
          </div>
    ...
    <div v-if="showTemplateManager" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl shadow-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-slate-200">
        <div class="p-6 border-b border-slate-200">
          <h3 class="text-lg font-semibold text-slate-900">Communication Templates</h3>
        </div>

        <div class="p-6">
          <p class="text-sm text-slate-600 mb-4">Manage your custom communication templates</p>
          <!-- Template list would go here in full implementation -->
          <p class="text-center text-slate-500 py-8 text-sm">Template management coming in next update</p>
        </div>
...
watch(selectedEmailTemplate, (templateId) => {
  if (templateId) {
    ...
    if (template) {
      emailComposer.value = {
        subject: template.subject ? interpolateText(template.subject, templateVars) : '',
...
watch(selectedTextTemplate, (templateId) => {
  if (templateId) {
    ...
    if (template) {
      textComposer.value = { body: interpolateTemplate(template, templateVars) }
    }
  }
})
