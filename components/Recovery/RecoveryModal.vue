    ...
    <Transition name="fade">
      <div v-if="isOpen" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-slide-up">
          ...
          <div class="bg-gradient-to-r from-amber-500 to-red-500 text-white px-6 py-8">
          ...
          <div class="px-6 py-6 space-y-6">
            ...
            <div class="flex items-center gap-3 text-sm">
              ...
              <div>
                <p class="font-semibold text-slate-900">{{ plan.duration_days }}-Day Plan</p>
                <p class="text-xs text-slate-600">Realistic timeline to get back on track</p>
              </div>
            </div>
            ...
            <div class="space-y-3">
              ...
              <ol class="space-y-2">
                <li v-for="(step, idx) in plan.steps" :key="idx" class="flex gap-3">
                  <span class="flex-shrink-0 w-6 h-6 bg-green-100 text-green-700 rounded-full flex items-center justify-center text-xs font-bold">
                    {{ idx + 1 }}
                  </span>
            ...
            <div class="bg-green-50 rounded p-4 text-center">
              <p class="text-sm text-green-900">
                <strong>You've got this!</strong> Many athletes successfully recover from setbacks. Focus on the first step.
              </p>
            </div>
          </div>
