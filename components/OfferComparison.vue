<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] flex flex-col bg-white">
      <!-- Header -->
      ...
      <div class="flex-1 overflow-auto p-6">
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b-2 border-slate-300">
                <th class="text-left px-4 py-3 font-semibold sticky left-0 bg-white text-slate-900">
                  School
                </th>
                <th class="text-left px-4 py-3 font-semibold text-slate-900">Offer Type</th>
                <th class="text-right px-4 py-3 font-semibold text-slate-900">Amount</th>
                <th class="text-right px-4 py-3 font-semibold text-slate-900">Percentage</th>
                <th class="text-left px-4 py-3 font-semibold text-slate-900">Deadline</th>
                <th class="text-left px-4 py-3 font-semibold text-slate-900">Status</th>
                <th class="text-left px-4 py-3 font-semibold text-slate-900">Conditions</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                <td class="px-4 py-4 font-semibold sticky left-0 bg-white text-slate-900">
                  {{ getSchoolName(offer.school_id) }}
                </td>
                <td class="px-4 py-4 text-slate-600">{{ getOfferTypeLabel(offer.offer_type) }}</td>
                <td
                  class="px-4 py-4 text-right font-semibold"
                  :class="isHighestAmount(offer) ? 'text-emerald-600 bg-emerald-50' : 'text-slate-900'"
                >
                  {{ offer.scholarship_amount ? `$${offer.scholarship_amount.toLocaleString()}` : '—' }}
                </td>
                <td
                  class="px-4 py-4 text-right font-semibold"
                  :class="isHighestPercentage(offer) ? 'text-emerald-600 bg-emerald-50' : 'text-slate-900'"
                >
                  {{ offer.scholarship_percentage ? `${offer.scholarship_percentage}%` : '—' }}
                </td>
                <td :class="isEarliestDeadline(offer) && offer.deadline_date ? 'px-4 py-4 font-semibold text-orange-600' : 'px-4 py-4 text-slate-600'">
                  <div v-if="offer.deadline_date">
                    <div>{{ formatDate(offer.deadline_date) }}</div>
                    <div class="text-xs mt-0.5 text-slate-600">{{ daysUntil(offer) }}d</div>
                  </div>
                  <div v-else>—</div>
                </td>
                <td class="px-4 py-4">
                ...
                <td class="px-4 py-4 max-w-xs text-slate-600">
                  <div v-if="offer.conditions" class="text-xs truncate" :title="offer.conditions">
                    {{ offer.conditions }}
                  </div>
                  <div v-else>—</div>
                </td>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div class="rounded-lg p-4 bg-emerald-100 border border-emerald-200">
            <p class="text-sm font-medium mb-1 text-emerald-600">Highest Amount</p>
          ...
          <div class="rounded-lg p-4 bg-purple-100 border border-purple-200">
            ...
            <p class="text-2xl font-bold text-purple-900">{{ maxPercentage ? `${maxPercentage}%` : '—' }}</p>
          </div>
          ...
          <div class="rounded-lg p-4 bg-orange-100 border border-orange-200">
            ...
            <p class="text-2xl font-bold text-orange-900">{{ minDays ? `${minDays}d` : '—' }}</p>
          </div>
        </div>
      </div>
...
const maxAmount = computed(() => {
  const amounts = props.offers.map((o: Offer) => o.scholarship_amount || 0).filter((a: number) => a > 0)
  return amounts.length > 0 ? Math.max(...amounts) : null
})

const maxPercentage = computed(() => {
  const percentages = props.offers.map((o: Offer) => o.scholarship_percentage || 0).filter((p: number) => p > 0)
  return percentages.length > 0 ? Math.max(...percentages) : null
})

const minDays = computed(() => {
  const days = props.offers.map((o) => daysUntil(o)).filter((d) => d !== null && d > 0) as number[]
  return days.length > 0 ? Math.min(...days) : null
})

const isHighestAmount = (offer: Offer): boolean => {
  return maxAmount.value !== null && offer.scholarship_amount === maxAmount.value
}

const isHighestPercentage = (offer: Offer): boolean => {
  return maxPercentage.value !== null && offer.scholarship_percentage === maxPercentage.value
}
