<template>
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
    ...
    <div class="rounded-lg shadow p-6 bg-white">
      <h3 class="text-sm font-medium mb-2 text-slate-600">Total Interactions</h3>
    ...
    <div class="rounded-lg shadow p-6 bg-white">
      ...
      <p class="text-xs mt-1 text-slate-600">{{ getMostCommonTypeCount() }} interactions</p>
    </div>
    ...
    <div class="rounded-lg shadow p-6 bg-white">
      ...
      <div class="space-y-1 text-xs">
        <p><span class="font-medium text-emerald-600">Very Positive:</span> {{ veryPositiveCount }}</p>
        <p><span class="font-medium text-blue-600">Positive:</span> {{ positiveCount }}</p>
        <p><span class="font-medium text-slate-600">Neutral:</span> {{ neutralCount }}</p>
      </div>
    </div>
  </div>
  ...
  <div class="rounded-lg shadow p-6 mb-8 bg-white">
    ...
    <div class="space-y-2">
      <div v-for="(count, type) in typeDistribution" :key="type" class="flex items-center gap-4">
        <span class="w-32 text-sm font-medium truncate text-slate-900">{{ formatType(type) }}</span>
        <div class="flex-1 h-6 rounded-full overflow-hidden bg-slate-50">
          <div
            :style="{ width: (count / interactions.length) * 100 + '%', backgroundColor: getTypeColorVar(type) }"
            class="h-full flex items-center justify-end pr-2"
          >
            <span v-if="(count / interactions.length) * 100 > 10" class="text-xs font-bold text-white">
