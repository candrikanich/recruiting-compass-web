<template>
  <div class="bg-white border rounded-xl p-4 shadow-sm" :class="urgencyClasses">
    <div class="flex items-start gap-3">
      <!-- Urgency icon -->
      <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" :class="iconClasses">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm font-medium text-slate-900 mb-2">{{ suggestion.message }}</p>
...
const actionClasses = computed(() => {
  switch (props.suggestion.urgency) {
    ...
    case 'medium': return 'bg-orange-600 text-white hover:bg-orange-700'
    case 'low': return 'bg-blue-600 text-white hover:bg-blue-700'
...
const actionLabel = computed(() => {
  switch (props.suggestion.action_type) {
    case 'log_interaction': return 'Log Interaction'
    case 'add_video': return 'Add Video'
    case 'add_school': return 'Add School'
    case 'update_video': return 'Update Video'
...
function handleAction() {
  switch (props.suggestion.action_type) {
    case 'log_interaction':
      navigateTo(`/interactions/add${props.suggestion.related_school_id ? `?schoolId=${props.suggestion.related_school_id}` : ''}`)
      break
    case 'add_video':
      navigateTo('/videos')
      break
