<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h3 class="text-lg font-semibold text-slate-900 mb-2">Coach Interest Level</h3>
      <p class="text-sm text-slate-600">
        Answer these questions to gauge the coach's level of interest based on their response.
      </p>
    </div>

    <!-- Questions -->
    <div class="space-y-4">
      <div v-for="(question, idx) in questions" :key="idx" class="flex items-start gap-3">
        <input
          type="checkbox"
        ...
        />
        <label :for="`calibration-q${idx}`" class="text-sm text-slate-700 cursor-pointer flex-1">
          {{ question }}
        </label>
      </div>
    </div>
