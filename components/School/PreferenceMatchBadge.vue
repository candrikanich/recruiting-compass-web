<template>
  <div v-if="badge" class="inline-flex items-center gap-1">
    ...
    <span v-if="showScore && !matchResult.hasDealbreakers" class="text-xs text-slate-600">
      {{ matchResult.score }}%
    </span>
  </div>
</template>
...
import { useSchoolMatching, type MatchResult } from '~/composables/useSchoolMatching'
...
const badge = computed(() => {
  return getMatchBadge(matchResult.value.score, matchResult.value.hasDealbreakers)
})
</script>
