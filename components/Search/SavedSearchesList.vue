<template>
  <div class="bg-white rounded-lg shadow p-6">
    <div class="flex items-center justify-between mb-4">
      ...
      <span v-if="favoritedSearches.length > 0" class="text-sm text-gray-600">{{ favoritedSearches.length }} favorite{{ favoritedSearches.length !== 1 ? 's' : '' }}</span>
    </div>
    ...
    <div v-if="favoritedSearches.length > 0" class="space-y-2 mb-6">
      ...
      >
        <svg class="h-4 w-4 text-yellow-600 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
        </svg>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-gray-900 truncate">{{ search.name }}</p>
          <p class="text-xs text-gray-600">{{ search.query }} • Used {{ search.use_count }} times</p>
        </div>
        <button
    ...
    <div v-if="otherSearches.length > 0" class="space-y-2">
      <p v-if="favoritedSearches.length > 0" class="text-xs font-semibold text-gray-500 uppercase">Other Searches</p>
      <div
    ...
    <div v-if="savedSearches.length === 0" class="text-center py-8">
      ...
      <p class="text-xs text-gray-500">Save frequently used searches for quick access</p>
    </div>
  </div>
</template>
...
const { savedSearches, favoritedSearches, toggleFavorite, deleteSavedSearch } = useSavedSearches()
