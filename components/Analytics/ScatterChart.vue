...
const chartData = computed((): ChartData<"scatter", DataPoint[]> => {
  const chartDatasets: ChartDataset<"scatter", DataPoint[]>[] =
...
const _quadrantLines = computed(() => {
  if (!showQuadrants.value || allDataPoints.value.length === 0)
    return { x: 0, y: 0 };

  const xValues = [...allDataPoints.value.map((p) => p.x)].sort(
    (a, b) => a - b,
  );
  const yValues = [...allDataPoints.value.map((p) => p.y)].sort(
    (a, b) => a - b,
  );
