<template>
  <div class="rounded-lg p-6 bg-white shadow-md">
    ...
    <div v-if="hasData" class="flex flex-col items-center">
      ...
      >
        <!-- Title -->
        <text x="200" y="30" text-anchor="middle" class="text-sm font-semibold" fill="#374151">
          {{ title }}
        </text>
      ...
      <div class="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        <div
          ...
          :style="{ borderColor: stage.color || defaultColors[index % defaultColors.length] }"
          @click="handleStageClick(index)"
          @mouseenter="hoveredStage = index"
          @mouseleave="hoveredStage = null"
        ...
        >
          ...
          <p class="text-2xl font-bold mt-1 text-slate-900">{{ stage.value }}</p>
          <p class="text-xs mt-2 text-slate-600">{{ getStagePercentage(index) }}% of total</p>
...
const hasData = computed(() => props.stages.length > 0 && props.stages.some(s => s.value > 0))
...
const metrics = computed(() => {
  const allEqual = props.stages.length > 1 && props.stages.every(stage => stage.value === props.stages[0].value)
  ...
  return props.stages.map((stage, index) => {
    ...
    if (props.stages.length === 1) {
      ...
      conversionRate = previousValue === 0 ? 0 : Math.round((stage.value / previousValue) * 100)
    }
    
    return {
      ...stage,
      conversionRate,
      percentage: firstStageValue === 0 ? 0 : Math.round((stage.value / firstStageValue) * 100)
    }
  })
})
