...
onMounted(async () => {
  ...
  if (!slug) return;
  try {
    const data = await $fetch<PublicProfileData>(`/api/public/profile/${slug}`);
    ...
    <template v-else>
      ...
      <div v-else-if="link" class="space-y-3">
        ...
        <div class="flex gap-2">
          <code
            class="flex-1 truncate rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600"
          >
            {{ trackingUrl }}
          </code>
          <button
            class="rounded-lg bg-gray-800 px-3 py-2 text-xs whitespace-nowrap text-white hover:bg-gray-700"
            @click="copyLink"
          >
      ...
      <div v-if="canSend" class="flex gap-2 pt-1">
        <button
          ...
          class="rounded-lg bg-slate-100 px-3 py-2 text-xs text-slate-700 hover:bg-slate-200 disabled:opacity-50"
          :disabled="sending"
