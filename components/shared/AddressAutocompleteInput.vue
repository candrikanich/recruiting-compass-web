...
const { suggestions, loading, search, selectSuggestion, clearSuggestions } =
  useAddressAutocomplete();
...
<template>
  <div class="space-y-3">
    ...
    <div>
      ...
      <input
        ...
        @input="
          emit('update:modelValue', {
            ...modelValue,
            zip: ($event.target as HTMLInputElement).value,
          })
        "
      />
    </div>
    ...
    <p v-if="isSelected && modelValue.latitude" class="text-xs text-green-600">
      ✓ Location confirmed ({{ modelValue.latitude?.toFixed(4) }},
      {{ modelValue.longitude?.toFixed(4) }})
    </p>
  </div>
</template>
