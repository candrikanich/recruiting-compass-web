...
<template>
  <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
    ...
    >
      <div>
        <label class="mb-1 block text-[11px] font-semibold text-slate-400"
          >Type</label
        >
        <select
    ...
    <div class="mb-4 flex gap-3">
      <div
        class="flex w-[100px] flex-col items-center rounded-lg border border-slate-200 bg-slate-50 px-4 py-2"
      >
        <p class="text-[11px] font-semibold text-slate-500 uppercase">Shown</p>
        <p class="text-xl font-bold text-slate-900">{{ filtered.length }}</p>
      </div>
      <div
        class="flex w-[100px] flex-col items-center rounded-lg border border-blue-200 bg-blue-50 px-4 py-2"
      >
        <p class="text-[11px] font-semibold text-blue-500 uppercase">Sent</p>
        <p class="text-xl font-bold text-blue-500">{{ outboundCount }}</p>
      </div>
      <div
        class="flex w-[100px] flex-col items-center rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2"
      >
        <p class="text-[11px] font-semibold text-emerald-500 uppercase">
          Received
        </p>
        ...
        <li v-for="interaction in filtered" :key="interaction.id">
          ...
          >
            ...
            <span class="flex w-[180px] shrink-0 items-center gap-2">
              ...
              <span class="min-w-0">
                <span
                  class="block truncate text-[13px] font-semibold text-slate-900"
                  >{{ formatType(interaction.type) }}</span
                >
                <span
            ...
            <span class="min-w-0 flex-1">
              ...
              >
              <span v-else class="block truncate text-[13px] text-slate-400"
                >-</span
              >
              <span
            ...
            <span
              class="flex w-[180px] shrink-0 items-center justify-between gap-2"
            >
              <time
                :datetime="interaction.occurred_at"
                class="text-[12px] whitespace-nowrap text-slate-500"
              >
