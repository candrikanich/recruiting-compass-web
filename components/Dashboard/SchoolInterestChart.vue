...
const chartData = computed(() => {
  ...
  const labels = ['Researching', 'Contacted', 'Interested', 'Offer Received', 'Committed']
  const data = [
  ...
  const colors = ['researching', 'contacted', 'interested', 'offer_received', 'committed']
...
onMounted(() => {
  if (chartCanvas.value && chartData.value) {
    ...
    const statusColorMap: Record<string, string> = {
      researching: getComputedStyle(root).getPropertyValue('--muted').trim() || '#ececf0',
      ...
      offer_received: getComputedStyle(root).getPropertyValue('--brand-orange-500').trim() || '#f97316',
      committed: getComputedStyle(root).getPropertyValue('--brand-emerald-600').trim() || '#059669',
    }

    const resolvedColors = chartData.value.colors.map((colorKey: string) => statusColorMap[colorKey] || '#999')
