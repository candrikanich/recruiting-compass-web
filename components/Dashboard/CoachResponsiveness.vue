  ...
  >
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
      ...
      <div class="text-center">
        ...
        <p class="text-2xl font-bold mt-2 text-emerald-600">
          {{ topResponsiveness }}%
        </p>
      </div>
      <div class="text-center">
    ...
    <div v-if="topCoaches.length > 0" class="border-t border-slate-200 pt-6">
      ...
      <div class="space-y-3">
        <div
          v-for="(coach, idx) in topCoaches.slice(0, 5)"
          :key="coach.id"
        ...
        >
          ...
          <div class="text-right">
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm font-semibold w-12 text-right text-slate-900"
                >{{ Math.round(coach.responsiveness_score) }}%</span
              >
            </div>
          </div>
        </div>
    ...
    >
      <h3 class="font-semibold mb-4 text-slate-900">
        🔴 Needs Follow-up (Low Responsiveness)
      </h3>
      <div class="space-y-3">
        <div
          v-for="coach in needsFollowUp"
          :key="coach.id"
          class="p-3 rounded-lg bg-red-50 border border-red-300"
        >
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              ...
              <p class="text-xs text-slate-600">
                {{ daysSinceContact(coach.last_contact_date) }}d since contact
              </p>
            </div>
          </div>
        </div>
...
const needsFollowUp = computed(() => {
  return props.coaches
    .filter((c) => c.responsiveness_score < 50)
    .sort((a, b) => a.responsiveness_score - b.responsiveness_score);
});
