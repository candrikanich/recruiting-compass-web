  ...
  <div v-if="suggestions.length > 0" class="mb-6">
    <div class="mb-4">
      ...
      <p class="text-slate-600 text-sm mt-1">
        {{ suggestions.length }} item{{ suggestions.length !== 1 ? 's' : '' }} need{{ suggestions.length !== 1 ? '' : 's' }} your attention
      </p>
    </div>

    <!-- Parent Notice -->
    <div v-if="isViewingAsParent" class="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
      <p class="text-sm text-amber-800">
        💡 These suggestions are for <strong>{{ athleteName || 'your athlete' }}</strong>.
