<template>
  <div class="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow p-6">
    ...
    <!-- Empty State -->
    <div v-if="upcomingEvents.length === 0" class="text-center py-8 text-slate-600">
      <p>No upcoming events scheduled</p>
      <NuxtLink to="/events" class="text-sm mt-2 inline-block text-blue-600 hover:text-blue-700">
        View all events →
      </NuxtLink>
    </div>
    ...
    <div v-else class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <p class="font-semibold truncate text-slate-900">{{ event.name }}</p>
              <p class="text-xs text-slate-600">{{ getEventTypeLabel(event.type) }}</p>
              <p class="text-xs mt-1 text-slate-600">{{ formatDate(event.start_date) }}</p>
              <p v-if="event.location" class="text-xs text-slate-600">📍 {{ event.location }}</p>
            </div>
            <span class="text-xs font-bold px-2 py-1 rounded whitespace-nowrap mt-0.5 bg-blue-100 text-blue-700">
      ...
      <div class="text-center pt-2">
        <NuxtLink to="/events" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
          View all events →
        </NuxtLink>
      </div>
    </div>
