<template>
  <div class="space-y-4">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <!-- Accepted Count -->
      <div v-if="acceptedCount > 0" class="p-4 bg-green-50 rounded-lg border border-green-200">
        <div class="text-sm text-slate-600">Accepted</div>
        <div class="text-2xl font-bold text-green-600">{{ acceptedCount }}</div>
      </div>

      <!-- Pending Count -->
      <div v-if="pendingCount > 0" class="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
        <div class="text-sm text-slate-600">Pending</div>
        <div class="text-2xl font-bold text-yellow-600">{{ pendingCount }}</div>
      </div>

      <!-- Declined Count -->
      <div v-if="declinedCount > 0" class="p-4 bg-red-50 rounded-lg border border-red-200">
        <div class="text-sm text-slate-600">Declined</div>
        <div class="text-2xl font-bold text-red-600">{{ declinedCount }}</div>
      </div>

      <!-- Total Scholarship Value -->
      <div v-if="totalScholarshipValue > 0" class="p-4 bg-blue-50 rounded-lg border border-blue-200">
        <div class="text-sm text-slate-600">Total Scholarship Value</div>
        <div class="text-2xl font-bold text-blue-600">{{ totalScholarshipValue }}%</div>
      </div>

      <!-- Average Offer -->
      <div v-if="averageOffer > 0" class="p-4 bg-purple-50 rounded-lg border border-purple-200">
        <div class="text-sm text-slate-600">Average Offer</div>
        <div class="text-2xl font-bold text-purple-600">{{ averageOffer }}%</div>
      </div>

      <!-- Upcoming Deadlines -->
      <div v-if="upcomingDeadlines.length > 0" class="p-4 bg-orange-50 rounded-lg border border-orange-200">
        <div class="text-sm text-slate-600">Upcoming Deadlines</div>
        <div class="text-2xl font-bold text-orange-600">{{ upcomingDeadlines.length }}</div>
      </div>
    </div>
  </div>
</template>
...
const totalScholarshipValue = computed(() => {
  return props.offers.reduce((sum, offer) => sum + (offer.scholarship_percentage || 0), 0);
});
