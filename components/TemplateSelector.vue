  ...
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col bg-white">
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <!-- Type Filter -->
        <div class="mb-6">
          <label class="block text-sm font-medium mb-3 text-slate-600">Message Type</label>
        ...
        <div v-if="loadingUnlockStatus" class="text-center py-8">
          <div class="animate-spin rounded-full h-8 w-8 border border-blue-300 border-t-blue-600 mx-auto" />
        </div>
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center gap-2">
                  <h3 class="font-semibold text-slate-900">{{ item.template.name }}</h3>
                  <span
                ...
                <p class="text-sm mt-1 text-slate-600">{{ item.template.description }}</p>

                <!-- Missing Requirements -->
                <div v-if="!item.unlocked && item.missingConditions.length > 0" class="mt-2">
                ...
                <!-- Progress Bar -->
                <div v-if="item.template.is_predefined && item.progressPercent < 100" class="mt-2">
                  <div class="text-xs text-slate-600 mb-1">Progress: {{ item.progressPercent }}%</div>
...
const initializeTemplates = async () => {
  ...
  try {
    ...
    templatesWithUnlock.value = await getTemplatesWithUnlockStatus(allTemplates)
  } catch (err) {
...
const filteredTemplates = computed(() => {
  return templatesWithUnlock.value.filter((item) => item.template.type === selectedType.value)
})
