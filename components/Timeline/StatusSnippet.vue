<template>
  <div
    v-if="!loading"
    class="bg-gradient-to-r border border-slate-200 rounded-xl p-4 mb-6 shadow-sm"
    :class="`${contextColor}`"
  >
    <div class="flex items-center justify-between">
      ...
      <div class="flex-1">
        <!-- Phase badge -->
        <div
          class="inline-block px-3 py-1 bg-white rounded-full text-sm font-medium text-slate-900 mb-2"
        >
...
const message = computed(() => {
  ...
  const contextMessages: Record<TimelineContext, Record<Phase, string>> = {
    schools: {
      freshman:
        "Research programs that match your academic and athletic profile",
  ...
  return (
    contextMessages[props.context]?.[currentPhase.value] ||
    "Check your recruiting timeline"
  );
});
