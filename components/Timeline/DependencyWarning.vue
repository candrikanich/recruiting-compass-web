<template>
  <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
    ...
    <div class="flex items-start gap-2 mb-2">
      ...
      <div class="flex-1">
        <div class="text-xs font-semibold text-yellow-900">This task works best after:</div>
      </div>
    </div>

    <!-- Prerequisites list -->
    <div class="ml-6 space-y-1">
      <div v-for="prerequisite in incompletePrerequisites" :key="prerequisite.id" class="text-xs text-yellow-800">
        <span class="font-medium">{{ prerequisite.title }}</span>
      </div>
    </div>

    <!-- Note about why it matters -->
    <div v-if="task.why_it_matters" class="mt-2 ml-6 text-xs text-yellow-700 italic">
      {{ task.why_it_matters }}
    </div>
...
const incompletePrerequisites = computed(() => {
  ...
  return props.task.prerequisite_tasks.filter(
    t => !props.task.athlete_task || props.task.athlete_task.status !== 'completed'
  )
})

const hasCompletePrerequisiteOption = computed(() => incompletePrerequisites.value.length > 0)
</script>
