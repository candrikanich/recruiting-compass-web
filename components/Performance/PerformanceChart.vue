  ...
  <div class="rounded-lg shadow p-6 bg-white">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
      <div>
        ...
        <p class="text-sm mt-1 text-slate-600">{{ metrics.length }} metrics recorded</p>
      </div>
    ...
    <div v-else class="text-center py-12">
      <p class="text-slate-600">No metrics recorded for the selected criteria</p>
    </div>

    <!-- Period Comparison (if enabled) -->
    <div v-if="hasData && showComparison" class="mt-6 pt-6 border-t border-slate-300">
      <h3 class="text-lg font-semibold mb-4 text-slate-900">Period Comparison (30 days)</h3>
...
const getUnit = (): string => {
  switch (props.category) {
    case 'power': return 'mph'
    case 'speed': return 'sec'
    case 'hitting': return 'avg'
    case 'pitching': return 'val'
    default: return 'val'
  }
}
...
const stats = computed(() => {
  ...
  const avg = analytics.calculateAverage(visibleMetricsData.value as Performance[], 'value')
  const max = analytics.calculateMax(visibleMetricsData.value as Performance[], 'value')
  const min = analytics.calculateMin(visibleMetricsData.value as Performance[], 'value')
  const trend = analytics.calculateTrend(visibleMetricsData.value as Performance[], 'value')

  // Calculate percent change from first to last
  const sorted = [...visibleMetricsData.value].sort((a, b) =>
...
const chartData = computed(() => {
  const datasets = visibleMetrics.value
    .map(type => {
