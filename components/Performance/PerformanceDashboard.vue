<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      <div
        class="rounded-lg border border-blue-200 bg-linear-to-br from-blue-100 to-blue-50 p-6"
      >
        ...
        <p class="mt-1 text-xs text-slate-600">across all categories</p>
      </div>

      <div
        class="rounded-lg border border-emerald-200 bg-linear-to-br from-emerald-100 to-emerald-50 p-6"
      >
        <p class="text-sm font-semibold text-slate-600 uppercase">
          Latest Record
        </p>
        ...
        <p class="mt-1 text-xs text-slate-600">{{ formatDate(latestDate) }}</p>
      </div>

      <div
        class="rounded-lg border border-purple-200 bg-linear-to-br from-purple-100 to-purple-50 p-6"
      >
        <p class="text-sm font-semibold text-slate-600 uppercase">
          Metric Types
        </p>
        ...
        <p class="mt-1 text-xs text-slate-600">being tracked</p>
      </div>

      <div
        class="rounded-lg border border-orange-200 bg-linear-to-br from-orange-100 to-orange-50 p-6"
      >
        <p class="text-sm font-semibold text-slate-600 uppercase">
          Overall Trend
        </p>
        <p class="mt-2 text-3xl font-bold" :class="getTrendColorClass()">
          {{ overallTrend }}
        </p>
    ...
    <div
      v-if="summaryMetricTypes.length > 0"
      class="rounded-lg bg-white p-6 shadow-sm"
    >
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Metric Summary</h3>
    ...
    <div class="rounded-lg bg-white p-6 shadow-sm">
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Recent Metrics</h3>
      <div v-if="recentMetrics.length > 0" class="space-y-2">
        <div
          v-for="metric in recentMetrics.slice(0, 5)"
          :key="metric.id"
          class="flex items-center justify-between rounded-sm bg-slate-50 px-3 py-2"
        >
          <div>
