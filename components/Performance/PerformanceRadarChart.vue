<template>
  <div class="rounded-lg shadow p-6 bg-white">
    <h2 class="text-2xl font-bold mb-4 text-slate-900">Current Performance Snapshot</h2>
    ...
    <div v-else class="text-center py-12">
      <p class="text-slate-600">Log at least one metric to see your performance snapshot</p>
    </div>
    ...
    <div v-if="hasMetrics" class="mt-8 pt-6 border-t border-slate-300">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="type in metricTypes" :key="type" v-show="latestMetrics[type]" class="text-sm">
          <div class="text-slate-600">{{ getMetricLabel(type) }}</div>
          <div class="text-lg font-bold text-slate-900">
            {{ latestMetrics[type]?.value.toFixed(2) }}
            <span class="text-sm text-slate-600">{{ latestMetrics[type]?.unit }}</span>
          </div>
        </div>
      </div>
    </div>
...
const metricTypes = ['velocity', 'exit_velo', 'sixty_time', 'pop_time', 'batting_avg', 'era', 'strikeouts', 'other']
