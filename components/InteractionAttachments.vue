<template>
  <div v-if="attachments && attachments.length > 0" class="mt-3 pt-3 border-t border-slate-300">
    <p class="text-xs font-semibold mb-2 text-slate-600">📎 Attachments ({{ attachments.length }})</p>
...
const getFileName = (filepath: string): string => {
  ...
  return withoutTimestamp.length > 20 ? withoutTimestamp.substring(0, 17) + '...' : withoutTimestamp
}
