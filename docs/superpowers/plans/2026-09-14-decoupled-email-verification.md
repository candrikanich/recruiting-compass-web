...
Supabase's native CAPTCHA / Attack-Protection stays **ON**. An earlier draft of this section recommended turning it off so the post-signup `signInWithPassword` wouldn't be rejected for replaying the already-consumed signup Turnstile token — but `login()` and `POST /api/auth/signup-minor` have no server-side Turnstile check of their own, so the native toggle is their only bot defense (see `login-turnstile-captcha-gap.md`, PR #700). Disabling it project-wide would strip that. Instead, `pages/signup.vue` now mints a **fresh** Turnstile token immediately before the post-signup sign-in (`getFreshTurnstileToken`, threaded into `useAuth().signup()` as the `getFreshCaptchaToken` callback), so no token is ever replayed and the native gate can stay enabled.
