...
describe('pages/schools/[id]/coaches.vue', () => {
  ...
  describe('Page Load', () => {
    ...
    it('should show Add Coach button', () => {
      ...
      const addButton = wrapper.findAll('button').find((btn) => btn.text().includes('+ Add Coach'))
...
it('should sort by responsiveness (highest first)', async () => {
  ...
  describe('Clear Filters', () => {
    ...
    it('should show clear filters button when filters are active', async () => {
      ...
      const clearButton = wrapper.findAll('button').find((btn) => btn.text() === 'Clear Filters')
    ...
    it('should hide clear filters button when no active filters', () => {
      ...
      const clearButtons = wrapper.findAll('button').filter((btn) => btn.text() === 'Clear Filters')
    ...
    it('should reset all filters when clicked', async () => {
      ...
      expect((wrapper.find('#roleFilter').element as HTMLSelectElement).value).toBe('')
      expect((wrapper.find('#sortFilter').element as HTMLSelectElement).value).toBe('name')
    })
  })
  ...
  describe('Results Summary', () => {
    it('should show correct count for filtered results', async () => {
      mockCoaches.value = [
        createMockCoach({ id: 'coach-1', first_name: 'John', last_name: 'Smith' }),
        createMockCoach({ id: 'coach-2', first_name: 'Jane', last_name: 'Doe' }),
        createMockCoach({ id: 'coach-3', first_name: 'Bob', last_name: 'Johnson' }),
      ]
...
it('should enable submit button when required fields are filled', async () => {
    ...
    it('should handle creation error', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to add coach:', expect.any(Error))
    })
  })
  ...
  describe('Delete Coach', () => {
    ...
    it('should confirm before deleting', async () => {
      ...
      expect(window.confirm).toHaveBeenCalledWith('Are you sure you want to delete this coach?')
    })
    ...
    it('should handle delete error', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to delete coach:', expect.any(Error))
    })
  })
