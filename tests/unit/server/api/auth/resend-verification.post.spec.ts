...
vi.stubGlobal("readBody", vi.fn(async () => ({ email: mockBodyState.email })))
...
const { default: handler } = await import(
  "~/server/api/auth/resend-verification.post"
)
...
describe("POST /api/auth/resend-verification — input validation", () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(rateLimitByIp).mockResolvedValue({ ...mockRateLimitState, success: true })
...
describe("POST /api/auth/resend-verification — user lookup", () => {
  ...
  it("returns already-verified message when user is already confirmed", async () => {
    mockListUsersByFilter.mockResolvedValue({
      data: {
        users: [{ email: "test@example.com", email_confirmed_at: "2026-01-01T00:00:00Z" }],
      },
      error: null,
    })
