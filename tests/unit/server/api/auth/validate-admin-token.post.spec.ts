...
vi.stubGlobal("readBody", vi.fn(async () => ({ token: mockState.token })))
vi.stubGlobal("useRuntimeConfig", vi.fn(() => ({ adminTokenSecret: mockState.adminTokenSecret })))
vi.stubGlobal("createError", createError)

import { validateAdminToken } from "~/server/utils/adminToken"

const { default: handler } = await import(
  "~/server/api/auth/validate-admin-token.post"
)
...
describe("POST /api/auth/validate-admin-token", () => {
  ...
  describe("happy path", () => {
    ...
    it("calls validateAdminToken with token and secret", async () => {
      ...
      expect(validateAdminToken).toHaveBeenCalledWith("my-token", "test-secret")
    })
  })
  ...
  describe("error handling", () => {
    ...
    it("re-throws HTTP errors as-is", async () => {
      const httpError = createError({ statusCode: 403, statusMessage: "Forbidden" })
      vi.mocked(validateAdminToken).mockImplementation(() => {
        throw httpError
      })
