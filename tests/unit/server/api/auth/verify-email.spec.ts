...
vi.mock("h3", async (importOriginal) => {
  ...
  return {
    ...
    readBody: vi.fn(async () => ({ token: "valid-otp-token" })),
...
vi.stubGlobal("readBody", vi.fn(async () => ({ token: "valid-otp-token" })))
...
const { default: handler } = await import(
  "~/server/api/auth/verify-email.post"
)
...
describe("POST /api/auth/verify-email — rate limiting", () => {
  ...
  it("calls rateLimitByIp with correct options (10 req / 1 h)", async () => {
