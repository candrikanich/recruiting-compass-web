...
vi.stubGlobal("useRuntimeConfig", vi.fn(() => ({ adminTokenSecret: mockAdminTokenState.adminTokenSecret })))
...
const { default: handler } = await import(
  "~/server/api/auth/admin-profile.post"
)
...
describe("POST /api/auth/admin-profile", () => {
  beforeEach(() => {
    ...
    vi.mocked(validateAdminToken).mockImplementation(() => mockAdminTokenState.isValid)
  ...
  describe("happy path", () => {
    ...
    it("calls validateAdminToken with the token from body", async () => {
      ...
      expect(validateAdminToken).toHaveBeenCalledWith("valid-token", "test-secret")
    })
