...
describe("POST /api/auth/change-email", () => {
  ...
  it("returns { success: true } with valid new email and correct password", async () => {
    const result = await handler(
      makeEvent({ newEmail: "newemail@example.com", currentPassword: "CorrectPass1!" }),
    )
  ...
  it("throws 401 when signInWithPassword returns an error", async () => {
    ...
    await expect(
      handler(makeEvent({ newEmail: "newemail@example.com", currentPassword: "WrongPass!" })),
    ).rejects.toMatchObject({ statusCode: 401 })
  })

  it("throws 400 when newEmail is not a valid email address", async () => {
    await expect(
      handler(makeEvent({ newEmail: "not-an-email", currentPassword: "CorrectPass1!" })),
  ...
  it("throws 500 when updateUserById returns an error", async () => {
    ...
    await expect(
      handler(makeEvent({ newEmail: "newemail@example.com", currentPassword: "CorrectPass1!" })),
