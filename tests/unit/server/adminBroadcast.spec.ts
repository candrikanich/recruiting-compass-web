...
vi.mock("~/server/utils/auth", () => ({
  requireAdmin: vi.fn(async () => ({ id: "admin-uuid", email: "admin@example.com" })),
}))
...
const { broadcastSchema } = await import("~/server/api/admin/notifications/broadcast.post")
...
describe("broadcastSchema validation", () => {
  describe("valid inputs", () => {
    ...
    it("accepts all valid notification types", () => {
      const types = ["follow_up_reminder", "deadline_alert", "weekly_digest", "event"] as const
      for (const type of types) {
        const result = broadcastSchema.safeParse({ target: "all", type, title: "Test" })
