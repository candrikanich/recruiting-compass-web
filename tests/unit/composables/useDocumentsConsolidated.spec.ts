...
describe("useDocumentsConsolidated", () => {
  ...
  describe("CRUD Operations", () => {
    ...
    it("should fetch single document by ID", async () => {
      ...
      vi.mocked(composable as any).getDocument = vi
        .fn()
        .mockResolvedValue(mockDocument);
  ...
  describe("File Operations", () => {
    it("should validate file type before upload", async () => {
      ...
      const isValid = composable.validateFileType(validFile, [
        "application/pdf",
      ]);
    ...
    it("should reject invalid file types", async () => {
      ...
      const isValid = composable.validateFileType(invalidFile, [
        "application/pdf",
      ]);
    ...
    it("should validate file size against limit", async () => {
      ...
      vi.mocked(composable as any).validateFileSize = vi
        .fn()
        .mockReturnValue(true);
  ...
  describe("Sharing & Permissions", () => {
    it("should share document with specific user", async () => {
      ...
      vi.mocked(composable as any).shareDocument = vi
        .fn()
        .mockResolvedValue(share);

      const result = await composable.shareDocument(
        "doc-123",
        "user-456",
        "view",
      );
    ...
    it("should update permission level for shared user", async () => {
      ...
      const result = await composable.updatePermission(
        "doc-123",
        "user-456",
        "edit",
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should handle storage errors gracefully", async () => {
      ...
      await expect(composable.uploadFile(file, "doc-123")).rejects.toThrow(
        "Storage",
      );
    });
