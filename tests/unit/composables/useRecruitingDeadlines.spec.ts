...
describe("useRecruitingDeadlines", () => {
  ...
  it("returns empty systemDeadlines when no sport or schools", async () => {
    const { systemDeadlines } = await setupComposable({
      sport: null,
      divisions: [],
      graduationYear: null,
    });
  ...
  it("includes SAT/ACT test dates regardless of sport/division", async () => {
    ...
    const testDates = systemDeadlines.value.filter(
      (d: UnifiedDeadline) => d.category === "test",
    );
  ...
  it("converts dead period starts to deadline entries with endDate", async () => {
    const { getSportCalendar } =
      await import("~/utils/recruitingCalendar/resolver");
    ...
    const deadPeriods = systemDeadlines.value.filter(
      (d: UnifiedDeadline) => d.category === "ncaa-period",
    );
  ...
  it("deduplicates milestones across divisions (SAT appears once)", async () => {
    ...
    const satDates = systemDeadlines.value.filter((d: UnifiedDeadline) =>
      d.label.includes("SAT"),
    );
  ...
  it("sets isStale true when current date > SEASON_END", async () => {
    ...
    const { isStale } = await setupComposable({
      sport: "Baseball",
      divisions: ["D1"],
      graduationYear: 2028,
    });
