...
describe('useFamilyInvitations', () => {
  ...
  describe('fetchInvitations', () => {
    it('sets invitations, clears error, and resets loading on success', async () => {
      const sampleInvites = [
        { id: 'inv-1', invited_email: 'a@test.com', role: 'player', expires_at: '2099-01-01', created_at: '2026-01-01' },
      ]
      mockFetchAuth.mockResolvedValueOnce({ invitations: sampleInvites })
      const { fetchInvitations, invitations, error, loading } = useFamilyInvitations()
  ...
  describe('revokeInvitation', () => {
    it('calls DELETE then re-fetches invitations on success', async () => {
      const remaining = [
        { id: 'inv-2', invited_email: 'b@test.com', role: 'parent', expires_at: '2099-01-01', created_at: '2026-01-01' },
      ]
      mockFetchAuth
      ...
      expect(mockFetchAuth).toHaveBeenCalledWith('/api/family/invitations/inv-1', { method: 'DELETE' })
  ...
  describe('resendInvitation', () => {
    ...
    it('sets error and re-throws when the POST invite call fails', async () => {
      ...
      await expect(resendInvitation('inv-123', 'owen@example.com', 'player')).rejects.toThrow('Server error')
