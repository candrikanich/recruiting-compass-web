...
describe('useOffers - Critical Offer Creation Flow', () => {
  ...
  describe('Offer Creation', () => {
    ...
    it('should include user_id in created offer', async () => {
      ...
      mockQuery.single.mockResolvedValue({ data: createMockOffer(), error: null })
    ...
    it('should validate deadline is after offer date', async () => {
      ...
      expect(() => validateDates(offerData)).toThrow('Deadline must be after offer date')
    })
  ...
  describe('Offer Retrieval & Fetching', () => {
    ...
    it('should sort offers by deadline', async () => {
      const mockOffers = [
        createMockOffer({ id: 'offer-1', deadline_date: new Date('2025-03-01') }),
        createMockOffer({ id: 'offer-2', deadline_date: new Date('2025-01-15') }),
        createMockOffer({ id: 'offer-3', deadline_date: new Date('2025-02-15') }),
      ]
  ...
  describe('Offer Updates', () => {
    ...
    it('should update scholarship amount', async () => {
      ...
      const { updateOffer } = useOffers()
      const result = await updateOffer('offer-1', { scholarship_amount: 75000 })
    ...
    it('should update deadline date', async () => {
      ...
      const { updateOffer } = useOffers()
      const result = await updateOffer('offer-1', { deadline_date: newDeadline })
  ...
  describe('Deadline Management', () => {
    it('should identify offers with approaching deadlines', () => {
      ...
      const urgent = offers.filter((o) => {
        const daysLeft =
          (new Date(o.deadline_date).getTime() - now.getTime()) / (24 * 60 * 60 * 1000)
        return daysLeft <= 7
      })
    ...
    it('should calculate days until deadline', () => {
      ...
      const daysLeft = Math.ceil(
        (new Date(offer.deadline_date).getTime() - now.getTime()) / (24 * 60 * 60 * 1000)
      )
