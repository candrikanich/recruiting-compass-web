...
describe('useCoachStore', () => {
  ...
  describe('State Management', () => {
    it('should initialize with empty state', () => {
      ...
      expect(coachStore.filters).toEqual({ schoolId: undefined, role: undefined, search: undefined })
    })
  })

  describe('fetchCoaches', () => {
    it('should fetch coaches for a specific school', async () => {
      const mockCoaches = [createMockCoach(), createMockCoach({ id: 'coach-2', role: 'assistant' })]
  ...
  describe('fetchAllCoaches', () => {
    it('should fetch all coaches', async () => {
      const mockCoaches = [createMockCoach(), createMockCoach({ id: 'coach-2', school_id: 'school-456' })]
      mockQuery.order.mockResolvedValue({ data: mockCoaches, error: null })
  ...
  describe('createCoach', () => {
    it('should create a new coach and add to state', async () => {
      ...
      const createdCoach = createMockCoach({ ...newCoachData, id: 'new-coach' })
      ...
      const result = await coachStore.createCoach('school-123', newCoachData as any)
  ...
  describe('updateCoach', () => {
    it('should update an existing coach', async () => {
      ...
      expect(mockQuery.update).toHaveBeenCalledWith(expect.objectContaining(updates))
      expect(mockQuery.eq).toHaveBeenCalledWith('id', 'coach-1')
    ...
    it('should clear error on successful update', async () => {
      coachStore.error = 'Previous error'
      mockQuery.single.mockResolvedValue({ data: createMockCoach(), error: null })
    ...
    it('should handle update error', async () => {
      ...
      await expect(coachStore.updateCoach('coach-1', {})).rejects.toThrow('Update failed')
  ...
  describe('deleteCoach', () => {
    ...
    it('should handle delete error', async () => {
      ...
      await expect(coachStore.deleteCoach('coach-1')).rejects.toThrow('Delete failed')
  ...
  describe('Getters', () => {
    beforeEach(() => {
      coachStore.coaches = [
        createMockCoach({ id: 'coach-1', school_id: 'school-1', role: 'head', responsiveness_score: 85 }),
        createMockCoach({ id: 'coach-2', school_id: 'school-2', role: 'assistant', responsiveness_score: 60 }),
        createMockCoach({ id: 'coach-3', school_id: 'school-1', role: 'recruiting', responsiveness_score: 90 }),
      ]
    })
    ...
    it('should filter coaches by school', () => {
      ...
      expect(school1Coaches.every(c => c.school_id === 'school-1')).toBe(true)
    })
    ...
    it('should sort coaches by last contact date', () => {
      const now = new Date()
      coachStore.coaches = [
        createMockCoach({ id: 'coach-1', last_contact_date: new Date(now.getTime() - 10 * 24 * 60 * 60 * 1000).toISOString() }),
        createMockCoach({ id: 'coach-2', last_contact_date: new Date(now.getTime() - 5 * 24 * 60 * 60 * 1000).toISOString() }),
        createMockCoach({ id: 'coach-3', last_contact_date: new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000).toISOString() }),
      ]
  ...
  describe('Filter Management', () => {
    ...
    it('should reset filters', () => {
      coachStore.setFilters({ schoolId: 'school-1', role: 'head', search: 'john' })
  ...
  describe('Edge Cases', () => {
    ...
    it('should not duplicate coaches on multiple fetches', async () => {
      ...
      expect(coachStore.coaches.filter(c => c.id === 'coach-1')).toHaveLength(1)
    })
    ...
    it('should handle updating non-existent coach in state', async () => {
      ...
      expect(coachStore.coaches.find(c => c.id === 'coach-999')).toBeUndefined()
    })
  })
})
