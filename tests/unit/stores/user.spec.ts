...
describe('useUserStore', () => {
  ...
  describe('initializeUser', () => {
    ...
    it('should handle error fetching user profile', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to initialize user:', mockError)
      expect(store.user).toBeNull()
      expect(store.isAuthenticated).toBe(false)
      expect(store.loading).toBe(false)
    })
    ...
    it('should handle session fetch error', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to initialize user:', expect.any(Error))
      expect(store.user).toBeNull()
      expect(store.isAuthenticated).toBe(false)
    })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle database returning null for user', async () => {
      ...
      // Even with session, if no user profile, should be unauthenticated
      expect(store.user).toBeNull()
      expect(store.isAuthenticated).toBe(false)
    })
  })
