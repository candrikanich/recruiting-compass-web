...
describe("PerformanceDashboard", () => {
  ...
  describe("recent metrics", () => {
    ...
    it("should show latest 5 metrics", () => {
      const recentItems = wrapper.findAll('[class*="bg-slate-50"]');
  ...
  describe("trend colors", () => {
    it("should return trend color class", () => {
      const colorClass = wrapper.vm.getTrendColorClass();
      expect(["text-green-600", "text-red-600", "text-gray-900"]).toContain(
        colorClass,
      );
    });
  });
  ...
  describe("date formatting", () => {
    it("should format dates correctly", () => {
      ...
      expect(formatted).toMatch(/Jan (1[4-5]|15), 2024/); // Allow for timezone differences
    });
  });
