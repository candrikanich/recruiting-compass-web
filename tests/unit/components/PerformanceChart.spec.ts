...
// Mock StatCard component to render its content
vi.mock("~/components/Performance/StatCard.vue", () => ({
  default: {
    name: "StatCard",
    template: `
    ...
    `,
    props: ["label", "value", "unit", "trend", "percent"],
  },
}));

// Mock performance analytics composable
vi.mock("~/composables/usePerformanceAnalytics", () => ({
  usePerformanceAnalytics: () => ({
    calculateAverage: vi.fn(() => 85),
    calculateMax: vi.fn(() => 100),
    calculateMin: vi.fn(() => 80),
    calculateTrend: vi.fn(() => "improving"),
    calculatePercentChange: vi.fn(() => 25),
...
describe("PerformanceChart", () => {
  ...
  describe("metrics filtering", () => {
    ...
    it("should calculate statistics based on visible metrics", async () => {
      ...
      expect(statText).toContain("Power Metrics");
      expect(statText).toContain("3 metrics recorded");
      expect(statText).toContain("Fastball Velocity");
      expect(statText).toContain("Exit Velocity");
    });
  });
  ...
  describe("data calculations", () => {
    ...
    it("should display numeric statistics correctly", () => {
      ...
      expect(wrapper.text()).toContain("3 metrics recorded"); // Using the 3 metrics from beforeEach
    });
  });
  ...
  describe("category-specific behavior", () => {
    it("should render speed metrics with correct units", async () => {
      await wrapper.setProps({
        ...
        metrics: [createMockMetric({ metric_type: "sixty_time", value: 7.2 })],
      });
