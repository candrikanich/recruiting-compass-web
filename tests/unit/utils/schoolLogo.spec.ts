...
describe("getSchoolLogo", () => {
  it("should return Clearbit URL for a valid website", () => {
    ...
    expect(logo).toBe(
      "https://logo.clearbit.com/www.ua.edu?size=128&format=png",
    );
  });
  ...
  it("should fallback to UI Avatars if no website is provided", () => {
    ...
    expect(logo).toContain(
      "ui-avatars.com/api/?name=University%20of%20Alabama",
    );
  });
  ...
  it("should handle complex domains with paths", () => {
    ...
    expect(logo).toBe(
      "https://logo.clearbit.com/sub.domain.com?size=128&format=png",
    );
  });
});
