...
describe('Data Transformations - Exports and Filtering', () => {
  ...
  describe('CSV Export', () => {
    it('should convert school data to CSV format', () => {
      ...
      const csvRows = schools.map((s) => [s.id, s.name, s.location, s.division, s.status].join(','))
    ...
    it('should export interactions with metadata', () => {
      ...
      const csvRows = interactions.map((i) =>
        [i.id, i.type, i.direction, i.sentiment, new Date(i.occurred_at).toLocaleDateString()]
          .join(',')
      )
  ...
  describe('Fuzzy Search', () => {
    ...
    it('should find matches regardless of case', () => {
      const schools = [
        createMockSchool({ id: 's1', name: 'Boston College' }),
      ]
