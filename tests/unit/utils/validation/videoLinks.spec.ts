...
import {
  createVideoLinkSchema,
  updateVideoLinkSchema,
} from "~/utils/validation/schemas";

describe("video-link schemas", () => {
  it("accepts a valid create payload", () => {
    const r = createVideoLinkSchema.safeParse({
      platform: "hudl",
      url: "https://hudl.com/x",
      title: "Fall reel",
      position: 0,
    });
  ...
  it("accepts the 'other' platform for sites we don't enumerate", () => {
    expect(
      createVideoLinkSchema.safeParse({
        platform: "other",
        url: "https://somevideosite.com/x",
        position: 0,
      }).success,
    ).toBe(true);
  });
  ...
  it("update allows partial fields", () => {
    expect(updateVideoLinkSchema.safeParse({ title: "New" }).success).toBe(
      true,
    );
  });
