...
describe("EventForm", () => {
      ...
      global: {
        stubs: {
          ...
          DesignSystemFormSelect: {
            template:
              '<select v-model="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)"><option v-for="opt in options" :key="opt.value" :value="opt.value">{{ opt.label }}</option></select>',
            props: [
              "modelValue",
              "label",
              "required",
          ...
          DesignSystemFormDateInput: {
            template:
              '<input type="date" v-model="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" />',
            props: ["modelValue", "label", "required", "disabled", "error"],
          },
          DesignSystemFormTimeInput: {
            template:
              '<input type="time" v-model="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" />',
  ...
  it("emits submit event with form data when valid", async () => {
    ...
    const typeSelect = selects.find(
      (s) => s.element.value === "" || s.html().includes("Select Type"),
    );
    ...
    const textInputs = wrapper.findAll(
      'input[type!="date"][type!="time"][type!="checkbox"][type!="number"][type!="url"]',
    );
  ...
  it("disables fields when loading", () => {
    ...
    const submitButton = wrapper
      .findAll("button")
      .find((b) => b.text().includes("Creating"));
  ...
  it("renders school options from props", () => {
    ...
    const schoolSelect = selects.find((s) => {
      const html = s.html();
      return (
        html.includes("University of Florida") || html.includes("Georgia Tech")
      );
    });
  ...
  it("shows appropriate button text based on loading state", () => {
    ...
    const submitButton = wrapper
      .findAll("button")
      .find((b) => b.text().includes("Create Event"));
    expect(submitButton).toBeDefined();

    const loadingWrapper = createWrapper({ loading: true });
    const loadingButton = loadingWrapper
      .findAll("button")
      .find((b) => b.text().includes("Creating"));
