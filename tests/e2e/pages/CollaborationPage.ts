...
export class CollaborationPage extends BasePage {
  ...
  async changeCollaboratorRole(collaboratorName: string, newRole: string) {
    await this.click(`[data-testid="collaborator-${collaboratorName}"] [data-testid="role-button"]`)
    await this.clickByText(newRole)
  }

  async removeCollaborator(collaboratorName: string) {
    await this.click(`[data-testid="collaborator-${collaboratorName}"] [data-testid="remove-button"]`)
  ...
  async unlinkAccount(accountEmail: string) {
    await this.click(`[data-testid="account-${accountEmail}"] [data-testid="unlink-button"]`)
