...
export class OffersPage extends BasePage {
  ...
  async clickCreateOffer() {
    ...
    await this.page
      .locator('[role="dialog"], [data-testid="school-select"]')
      .first()
  ...
  async createOffer(offer: any) {
    ...
    await this.fillInput(
      ...
      offer.scholarshipAmount.toString(),
    );
    await this.fillInput(
      '[data-testid="scholarship-percentage-input"]',
      offer.scholarshipPercentage.toString(),
    );
  ...
  async acceptOffer(schoolName: string) {
    await this.click(
      `[data-testid="offer-${schoolName}"] [data-testid="accept-button"]`,
    );
  ...
  async declineOffer(schoolName: string) {
    await this.click(
      `[data-testid="offer-${schoolName}"] [data-testid="decline-button"]`,
    );
  ...
  async expectOfferStatus(schoolName: string, status: string) {
    await this.expectVisible(
      `[data-testid="offer-${schoolName}-status"]:has-text("${status}")`,
    );
  }
  ...
  async compareOffers() {
    await this.click('[data-testid="compare-offers-button"]');
    await this.waitForURL("/offers/compare");
  }

  async getOfferCount(): Promise<number> {
    // Look for offer cards or list items
    const cards = await this.page
      .locator('[data-testid="offer-card"], .offer-card, .card')
      .count();
    return cards;
  }
  ...
  async updateOffer(schoolName: string, updates: any) {
    if (updates.scholarshipAmount) {
      await this.fillInput(
        '[data-testid="scholarship-amount-input"]',
        updates.scholarshipAmount.toString(),
      );
    }
  ...
  async deleteOffer(schoolName: string) {
    await this.click(
      `[data-testid="offer-${schoolName}"] [data-testid="delete-button"]`,
    );
