...
const SEED_DATA = {
  ...
  coaches: [
    { first_name: 'John', last_name: 'Smith', role: 'Head Coach', email: 'jsmith@duke.edu' },
    {
      first_name: 'Mike',
      last_name: 'Johnson',
    ...
    { first_name: 'Sarah', last_name: 'Williams', role: 'Head Coach', email: 'swilliams@bc.edu' },
    { first_name: 'Tom', last_name: 'Brown', role: 'Assistant Coach', email: 'tbrown@bc.edu' },
    {
      first_name: 'David',
      last_name: 'Martinez',
      role: 'Head Coach',
...
async function seedDatabase() {
  ...
  try {
    ...
    const resetSQL = readFileSync(join(__dirname, 'sql/001-reset-tables.sql'), 'utf-8');
    ...
    try {
      ...
      console.log('⚠️  Could not clear data via API, proceeding with fresh seed');
    }
    ...
    const { data: schoolData, error: schoolError } = await supabase
      .from('schools')
      .insert(SEED_DATA.schools.map((s) => ({ ...s, created_at: new Date().toISOString() })))
      .select();
