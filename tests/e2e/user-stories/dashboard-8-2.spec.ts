...
test.describe("User Story 8.2: Contact Frequency Summary", () => {
  ...
  test("AC1: Scenario 1 - Contact Summary Metrics displayed correctly", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();

    // Verify all 4 summary metrics are visible
    await expect(page.locator('[data-testid="metric-total-schools"]')).toBeVisible();
    await expect(page.locator('[data-testid="metric-contacted-7days"]')).toBeVisible();
    await expect(page.locator('[data-testid="metric-avg-frequency"]')).toBeVisible();
    await expect(page.locator('[data-testid="metric-need-attention"]')).toBeVisible();

    // Verify metrics contain valid data
    const totalSchools = await page.locator('[data-testid="metric-total-schools"]').textContent();
    const contacted7Days = await page.locator('[data-testid="metric-contacted-7days"]').textContent();
    const avgFrequency = await page.locator('[data-testid="metric-avg-frequency"]').textContent();
    const needAttention = await page.locator('[data-testid="metric-need-attention"]').textContent();
  ...
  test("AC1: Scenario 1 - Metrics show correct labels", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="metric-total-schools"]')).toContainText("Total Schools");
    await expect(page.locator('[data-testid="metric-contacted-7days"]')).toContainText("Last 7 Days");
    await expect(page.locator('[data-testid="metric-avg-frequency"]')).toContainText("Avg/Month");
    await expect(page.locator('[data-testid="metric-need-attention"]')).toContainText("Need Attention");
  });

  test("AC2: Scenario 2 - Green border for recent contacts (within 7 days)", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();
    ...
    const schoolRows = contactFreqWidget.locator('[data-testid^="contacted-school-"]');
  ...
  test("AC2: Scenario 2 - Yellow border for schools contacted 8-30 days ago", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();
  ...
  test("AC2: Scenario 2 - Red border classes applied for old contacts", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();
  ...
  test("AC3: Scenario 3 - Clicking school navigates to school detail page", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();
  ...
  test("AC3: Scenario 3 - School detail page shows quick action buttons", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();
  ...
  test("AC4: Real-time Updates - Metrics update after logging interaction", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();

    // Get initial contacted 7 days count
    const initialCount = await page.locator('[data-testid="metric-contacted-7days"]').textContent();
  ...
  test("Contact Frequency widget displays metrics section", async ({ page }) => {
    dashboardPage = new DashboardPage(page);
    await dashboardPage.goto();
    await dashboardPage.waitForDashboardLoad();
  ...
  test("Summary metrics are accurate for tracked schools", async ({ page }) => {
    ...
    const totalSchoolsText = await page.locator('[data-testid="metric-total-schools"]').textContent();
    const totalSchools = parseInt(totalSchoolsText || "0");

    // Get contacted in 7 days count
    const contacted7DaysText = await page.locator('[data-testid="metric-contacted-7days"]').textContent();
