...
test.describe("Tier 1: Complete Recruiting Workflow", () => {
  ...
  test("should complete full workflow: school -> coach -> interaction", async ({
    ...
    await schoolsPage.expectSchoolDetails(
      testSchools.school1.name,
      testSchools.school1.location,
    );
    ...
    await coachesPage.expectCoachInList(
      testCoaches.coach1.firstName,
      testCoaches.coach1.lastName,
    );
  ...
  test("should add multiple coaches to same school", async ({ page }) => {
    ...
    await coachesPage.expectCoachInList(
      testCoaches.coach3.firstName,
      testCoaches.coach3.lastName,
    );
  ...
  test("should log multiple interactions with different types", async ({
    page,
  }) => {
    // Setup
    await schoolsPage.navigateToSchools();
  ...
  test("should update school status after receiving offer", async ({
    page,
  }) => {
