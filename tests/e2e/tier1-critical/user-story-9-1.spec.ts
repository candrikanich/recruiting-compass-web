...
test.describe('User Story 9.1 - Athlete Views Their Task List', () => {
  ...
  test('Scenario 1: Athlete views tasks for their grade', async ({ page }) => {
    ...
    if (hasAnyTasks > 0) {
      ...
      const taskTitle = page.locator('[data-testid="task-item"]').first().locator('h3');
  ...
  test('Task checkbox toggles visual state immediately', async ({ page }) => {
    ...
    const statusBadge = firstCheckbox.locator('xpath=../..//following-sibling::div[contains(@class, "rounded-full")]').first();
    await expect(statusBadge).toBeVisible();
  });
  ...
  test('Navigation between task list and other pages works', async ({ page }) => {
