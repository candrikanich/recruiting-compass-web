...
test.describe("Tier 2: Performance Tracking & Analytics", () => {
  ...
  test("should log performance metrics", async () => {
    ...
    await performancePage.expectMetricLogged(
      testPerformanceMetrics.metric1.type,
      testPerformanceMetrics.metric1.value.toString(),
    );
  });
  ...
  test("should view performance trends", async () => {
    ...
    await performancePage.clickAddMetric();
    await performancePage.logMetric({
      ...testPerformanceMetrics.metric1,
      value: 93,
    });
  ...
  test("should log multiple metric types", async () => {
    ...
    for (const metric of [
      testPerformanceMetrics.metric1,
      testPerformanceMetrics.metric2,
      testPerformanceMetrics.metric3,
    ]) {
      await performancePage.clickAddMetric();
