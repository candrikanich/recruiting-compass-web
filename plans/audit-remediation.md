...
- **Authorization model** (revised 2026-07-28, product decision, post-Phase 10a): `family_units` / `family_members` is authoritative wherever consolidation was proven safe — `schools` SELECT/INSERT/UPDATE, `users` SELECT, `interactions` UPDATE/DELETE, and `events` SELECT/UPDATE/DELETE. On the remaining tables/verb-groups, legacy `account_links`-era RLS policies remain load-bearing and are intentionally **not** removed, because app code doesn't yet populate `family_unit_id` on insert (or, for DELETE, no family-model policy exists at all to fall back to): `coaches`, `documents`, `performance_metrics`, `social_media_posts` (all verbs); `interactions` SELECT/INSERT; DELETE on `schools`/`coaches`/`documents`/`performance_metrics`. See `claude/database.md` for the durable, tracked list and the backfill precondition before any further consolidation. Server-side ownership checks remain mandatory on every service-role endpoint.
