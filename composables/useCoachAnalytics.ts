...
export const useCoachAnalytics = () => {
  ...
  const calculateCoachMetrics = (coachId: string): CoachMetrics => {
    const coachInteractions = interactions.value.filter((i) => i.coach_id === coachId)

    const outboundCount = coachInteractions.filter((i) => i.direction === 'outbound').length
    const inboundCount = coachInteractions.filter((i) => i.direction === 'inbound').length
    ...
    for (let i = 0; i < coachInteractions.length - 1; i++) {
      ...
      if (current.direction === 'outbound' && current.occurred_at) {
        const nextInbound = coachInteractions.slice(i + 1).find((x) => x.direction === 'inbound' && x.occurred_at)
        if (nextInbound && nextInbound.occurred_at) {
          const responseMs =
            new Date(nextInbound.occurred_at).getTime() - new Date(current.occurred_at).getTime()
    ...
    const averageResponseTime =
      responseCount > 0 ? Math.round((totalResponseTime / responseCount / (1000 * 60 * 60)) * 10) / 10 : 0

    // Last contact date
    const lastContact = coachInteractions.length > 0 ? coachInteractions[0].occurred_at || null : null
    ...
    const preferredMethod = Object.entries(methodCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || 'email'

    // Responsiveness score (same calculation as coachResponsiveness.ts)
    const responsiveness = outboundCount > 0 ? Math.min(Math.round(responseRate), 100) : 0
   ...
   */
  const calculateTrendData = (coachId: string, days: number = 90): TrendDataPoint[] => {
    ...
    Object.entries(dailyData)
      ...
      .forEach(([date, { outbound, inbound }]) => {
        ...
        const score =
          cumulativeOutbound > 0 ? Math.min(Math.round((cumulativeInbound / cumulativeOutbound) * 100), 100) : 0
   ...
   */
  const compareWithSchoolAverage = (coachId: string, schoolId: string | undefined) => {
    ...
    const avgResponseRate = schoolMetrics.length > 0
      ? Math.round(schoolMetrics.reduce((sum, m) => sum + m.responseRate, 0) / schoolMetrics.length)
      : 0
    const avgResponsiveness = schoolMetrics.length > 0
      ? Math.round(schoolMetrics.reduce((sum, m) => sum + m.responsiveness, 0) / schoolMetrics.length)
      : 0
    ...
    return {
      ...
      rank: schoolMetrics.filter((m) => m.responsiveness > coachMetrics.responsiveness).length + 1,
  ...
  const generateInsights = (coachId: string): string[] => {
    ...
    if (metrics.daysSinceContact > 30 && metrics.daysSinceContact >= 0) {
      insights.push(`No contact in ${metrics.daysSinceContact} days - consider reaching out`)
    }

    // Low responsiveness
    if (metrics.responsiveness < 25) {
      insights.push('Very low responsiveness - this coach rarely responds to outreach')
    } else if (metrics.responsiveness < 50) {
      insights.push('Low responsiveness - consider trying a different communication method')
    }
    ...
    if (metrics.averageResponseTime > 48) {
      insights.push(`Average response time is ${metrics.averageResponseTime} hours - slow responder`)
    } else if (metrics.averageResponseTime > 0 && metrics.averageResponseTime < 24) {
      insights.push(`Quick responder - average ${metrics.averageResponseTime} hours`)
    }
