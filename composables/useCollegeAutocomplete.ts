import { ref } from 'vue'
import type { CollegeSearchResult, CollegeScorecardResponse, CollegeScorecardSchool } from '~/types/api'
...
export const useCollegeAutocomplete = () => {
   ...
   */
  const transformResult = (school: CollegeScorecardSchool): CollegeSearchResult => {
  ...
  const searchColleges = async (query: string): Promise<void> => {
    ...
    if (!apiKey) {
      error.value = 'College search is not configured. Please enter the school manually.'
      return
    }
    ...
    try {
      const params = new URLSearchParams({
        api_key: apiKey,
        'school.name': query,
        fields: 'id,school.name,school.city,school.state,school.school_url,location.lat,location.lon,latest.admissions.admission_rate.overall,latest.student.size,program_percentage.mathematics,enrollment.all,net_price.public.by_income_level.110001-plus,tuition.in_state,tuition.out_of_state,alias,program_reporter.programs_offered,booksupply,roomboard.oncampus,otherexpense.oncampus,main_campus,carnegie_size_setting',
        per_page: '10',
      })
      ...
      if (!response.ok) {
        if (response.status === 401) {
          error.value = 'College Scorecard API key is invalid. Please check your configuration.'
