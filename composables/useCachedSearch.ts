...
export const useCachedSearch = () => {
  ...
  const getRefValue = <T,>(ref: any): T[] => {
    return (ref && typeof ref === 'object' && 'value' in ref && Array.isArray(ref.value)) ? ref.value : []
  }
  ...
  const performCachedSearch = async (searchQuery: string) => {
    ...
    const searchType = getRefValue<string>(searchComposable.searchType)[0] || 'all'
    const cacheId = cacheKey.search(
      userStore.user.id,
      searchQuery,
      searchType
    )
    ...
    const intRes = getRefValue<Interaction>(searchComposable.interactionResults)
    const metRes = getRefValue<PerformanceMetric>(searchComposable.metricsResults)
  ...
  const schoolResults = computed(() => getRefValue<School>(searchComposable.schoolResults))
  const coachResults = computed(() => getRefValue<Coach>(searchComposable.coachResults))
  const interactionResults = computed(() => getRefValue<Interaction>(searchComposable.interactionResults))
  const metricsResults = computed(() => getRefValue<PerformanceMetric>(searchComposable.metricsResults))
