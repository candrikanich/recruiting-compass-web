...
export const useCommunicationTemplates = (): {
  ...
  createTemplate: (name: string, type: CommunicationTemplate['type'], body: string, subject?: string, description?: string, tags?: string[]) => Promise<CommunicationTemplate | null>
  updateTemplate: (id: string, updates: Partial<CommunicationTemplate>) => Promise<boolean>
  deleteTemplate: (id: string) => Promise<boolean>
  toggleFavorite: (id: string) => Promise<boolean>
  incrementUseCount: (id: string) => Promise<void>
  renderTemplate: (template: CommunicationTemplate, variables: Record<string, string>) => string
  interpolateTemplate: (template: CommunicationTemplate, variables: Record<string, string>) => string
  getTemplate: (id: string) => CommunicationTemplate | undefined
  getTemplatesByType: (type: CommunicationTemplate['type']) => CommunicationTemplate[]
  searchTemplates: (query: string, type?: CommunicationTemplate['type']) => CommunicationTemplate[]
} => {
  ...
  const loadTemplates = async () => {
    ...
    try {
      ...
      if (err) {
        ...
        if (err.code === 'PGRST205' || err.message?.includes('communication_templates')) {
          templates.value = []
          return
        }
        throw err
  ...
  const renderTemplate = (template: CommunicationTemplate, variables: Record<string, string>): string => {
  ...
  // Search templates
  const searchTemplates = (query: string, type?: CommunicationTemplate['type']): CommunicationTemplate[] => {
  ...
  const getTemplatesByType = (type: CommunicationTemplate['type']): CommunicationTemplate[] => {
    return templates.value.filter(t => t.type === type)
  }

  // Interpolate template (alias for renderTemplate for backward compatibility)
  const interpolateTemplate = (template: CommunicationTemplate, variables: Record<string, string>): string => {
