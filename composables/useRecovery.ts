...
export function useRecovery() {
  ...
  const checkCriticalTaskMissed = (): RecoveryTrigger | null => {
    ...
    const missedCritical = criticalTaskIds.filter(
  ...
  const checkNoCoachInterest = (): RecoveryTrigger | null => {
    ...
    if (recentPositive.length === 0) {
      ...
      const lastIntDate = new Date(
        lastInteraction.occurred_at || lastInteraction.created_at || "",
      );
      const daysAgo = Math.floor(
        (Date.now() - lastIntDate.getTime()) / (1000 * 60 * 60 * 24),
      );
   ...
   */
  const checkEligibilityIncomplete =
    async (): Promise<RecoveryTrigger | null> => {
      try {
        ...
        const eligibilityTask = athleteTasks.value.find(
          (at) => at.task_id === "task-11-a1",
        );
    ...
    if (!schools.value || schools.value.length < 3) {
      return {
        ...
        details: {
          ...
          matchSafetyCount: 0,
          recommendation:
            "Build a balanced college list with reach, match, and safety schools",
        },
        severity: "high",
      };
    }
  ...
  const calculateDaysUntilDeadline = (): number => {
    ...
    const deadline =
      now < thisYearDeadline ? thisYearDeadline : nextYearDeadline;
