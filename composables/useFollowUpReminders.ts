...
type FollowUpReminderInsert = Database['public']['Tables']['follow_up_reminders']['Insert']
type FollowUpReminderUpdate = Database['public']['Tables']['follow_up_reminders']['Update']
...
export const useFollowUpReminders = () => {
  ...
  const upcomingReminders = computed(() => {
    ...
    return activeReminders.value.filter(
      r => new Date(r.due_date) >= now && new Date(r.due_date) <= oneWeekFromNow
    )
  })
  ...
  const getRemindersFor = (entityType: 'school' | 'coach' | 'interaction', entityId: string): FollowUpReminder[] => {
    const key = entityType === 'school' ? 'school_id' : entityType === 'coach' ? 'coach_id' : 'interaction_id'
    return reminders.value.filter(r => r[key as keyof FollowUpReminder] === entityId)
  }
  ...
  const formatDueDate = (dueDate: string): string => {
    ...
    if (dateAtMidnight < today) {
      const daysOverdue = Math.floor((today.getTime() - dateAtMidnight.getTime()) / (24 * 60 * 60 * 1000))
      return 'Overdue by ' + daysOverdue + ' day(s)'
    }
    if (dateAtMidnight <= oneWeekFromNow) {
      const daysUntil = Math.floor((dateAtMidnight.getTime() - today.getTime()) / (24 * 60 * 60 * 1000))
