...
export const useSocialMedia = () => {
  ...
  const createPost = async (postData: Omit<SocialPost, 'id' | 'created_at' | 'updated_at'>) => {
    loading.value = true
    error.value = null
  ...
  const updatePost = async (id: string, updates: Partial<SocialPost>) => {
    ...
    try {
      ...
      if (sanitizedUpdates.post_content) {
        sanitizedUpdates.post_content = sanitizeHtml(sanitizedUpdates.post_content)
      }
      if (sanitizedUpdates.author_name) {
        sanitizedUpdates.author_name = sanitizeHtml(sanitizedUpdates.author_name)
      }
      if (sanitizedUpdates.author_handle) {
        sanitizedUpdates.author_handle = sanitizeHtml(sanitizedUpdates.author_handle)
      }
  ...
  // Computed filters
  const twitterPosts = computed(() => posts.value.filter((p) => p.platform === 'twitter'))
  const instagramPosts = computed(() => posts.value.filter((p) => p.platform === 'instagram'))
  const flaggedPosts = computed(() => posts.value.filter((p) => p.flagged_for_review))
  const recruitingPosts = computed(() => posts.value.filter((p) => p.is_recruiting_related))
