...
export const useCoachAvailability = () => {
   ...
   */
  const updateCoachAvailability = async (coachId: string, availability: CoachAvailability) => {
    const coach = coaches.value.find((c) => c.id === coachId)
   ...
   */
  const setDayAvailability = async (coachId: string, day: string, dayAvailability: DayAvailability) => {
  ...
  const removeBlackoutDate = async (coachId: string, date: string) => {
    const availability = getCoachAvailability(coachId)
    availability.blackout_dates = availability.blackout_dates.filter((d) => d !== date)
    return updateCoachAvailability(coachId, availability)
  }
   ...
   */
  const isAvailableAt = (coachId: string, date: Date, timeStr?: string): boolean => {
    const availability = getCoachAvailability(coachId)
    ...
    const dayData = availability[dayName as keyof CoachAvailability] as DayAvailability | undefined
  ...
  const getAvailabilitySummary = (coachId: string): string => {
    ...
    const available = DAYS.filter((day) => {
      const dayData = availability[day as keyof CoachAvailability] as DayAvailability | undefined
      return dayData?.available
    })

    if (available.length === 0) return 'Not available'
    if (available.length === 7) return 'Available daily'
    if (available.length === 5 && !available.includes('saturday') && !available.includes('sunday')) {
