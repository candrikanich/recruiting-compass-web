...
export const useCollaboration = () => {
  ...
  const mySharedRecords = computed(() => {
    return sharedRecords.value.filter(r => r.owner_user_id === userStore.user?.id)
  })

  const sharedWithMe = computed(() => {
    return sharedRecords.value.filter(r => r.shared_with_user_id === userStore.user?.id)
  })
  ...
  const loadSharedRecords = async () => {
    ...
    try {
      const { data, error: err } = await supabase
        .from('shared_records')
        .select('*')
        .or(`owner_user_id.eq.${userStore.user.id},shared_with_user_id.eq.${userStore.user.id}`)
        .order('shared_at', { ascending: false })

      if (err) throw err
      sharedRecords.value = data || []
    } catch (err) {
      error.value = err instanceof Error ? err.message : 'Failed to load shared records'
  ...
  const shareRecord = async (
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to share record'
  ...
  const updateAccessLevel = async (
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to update access level'
