...
export const useTasks = (): {
  ...
  updateTaskStatus: (taskId: string, status: TaskStatus) => Promise<AthleteTask>
  getTaskDependencies: (taskId: string) => Promise<Task[]>
  checkDependenciesComplete: (taskId: string) => Promise<{ complete: boolean; incompletePrerequisites: Task[] }>
  getRequiredTasksForPhase: (phase: Phase) => Task[]
  calculateTaskCompletionRate: (gradeLevel: number) => number
  getTaskWithDependencyWarning: (taskId: string) => Promise<TaskDependencyAnalysis>
  ...
  const fetchAthleteTasks = async (): Promise<AthleteTask[]> => {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch athlete tasks'
    ...
    try {
      ...
      const merged = await Promise.all(
        allTasks.map(async (task: Task) => {
          ...
          const allDepsComplete = dependencies.length === 0 ||
          ...
          return {
            ...
            has_incomplete_prerequisites: task.dependency_task_ids.length > 0 && !allDepsComplete,
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch tasks with status'
   ...
   */
  const updateTaskStatus = async (taskId: string, status: TaskStatus): Promise<AthleteTask> => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      // Update local state
      const athleteTaskIndex = athleteTasks.value.findIndex(at => at.task_id === taskId)
   ...
   */
  const checkDependenciesComplete = async (taskId: string): Promise<{
    complete: boolean
    ...
    try {
      ...
      const incompleteTasks = task.dependency_task_ids
        .map(depId => {
          const athleteTask = athleteTasks.value.find(at => at.task_id === depId)
  ...
  const getRequiredTasksForPhase = (phase: Phase): Task[] => {
    ...
    return tasks.value.filter(t => t.grade_level === gradeLevel && t.required)
  }
  ...
  const calculateTaskCompletionRate = (gradeLevel: number): number => {
    const gradeTasks = tasks.value.filter(t => t.grade_level === gradeLevel && t.required)
  ...
  const getTaskWithDependencyWarning = async (
    ...
    try {
      ...
      const { complete, incompletePrerequisites } = await checkDependenciesComplete(taskId)
      ...
      if (!complete && incompletePrerequisites.length > 0) {
        ...
        warning = {
          ...
          whyItMatters: prerequisite.why_it_matters || 'Complete this prerequisite first.',
        }
      }
  ...
  const getCompletionStats = (gradeLevel: number): TaskCompletionStats => {
    const gradeTasks = tasksWithStatus.value.filter(t => t.grade_level === gradeLevel)

    const stats: TaskCompletionStats = {
      total: gradeTasks.length,
      completed: gradeTasks.filter(t => t.athlete_task?.status === 'completed').length,
      inProgress: gradeTasks.filter(t => t.athlete_task?.status === 'in_progress').length,
      notStarted: gradeTasks.filter(t => t.athlete_task?.status === 'not_started' || !t.athlete_task).length,
      skipped: gradeTasks.filter(t => t.athlete_task?.status === 'skipped').length,
      percentComplete: 0,
    }
  ...
  const requiredCompletedTasks = computed(() =>
    tasks.value.filter(
      t => t.required && athleteTasks.value.find(at => at.task_id === t.id && at.status === 'completed')
    )
  )
