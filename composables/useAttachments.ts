...
export const useAttachments = () => {
  ...
  const getDownloadUrl = (filepath: string): string => {
    const { data } = supabase.storage.from('interaction-attachments').getPublicUrl(filepath)
  ...
  const deleteAttachment = async (filepath: string): Promise<boolean> => {
    try {
      const { error } = await supabase.storage.from('interaction-attachments').remove([filepath])
