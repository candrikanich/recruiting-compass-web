...
export const useCollegeScorecardCache = () => {
   ...
   */
  const fetchScorecardData = async (schoolName: string, apiKey?: string): Promise<ScorecardData | null> => {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to fetch scorecard data'
      return null
    } finally {
      loading.value = false
    }
