...
import type { FitScoreResult, PortfolioHealth, FitTier, FitScoreInputs } from '~/types/timeline'
import {
  calculateFitScore,
...
export const useFitScore = (): {
  ...
  calculateSchoolFitScore: (schoolId: string, athleteData?: { position?: string; height?: number; weight?: number; velo?: number; gpa?: number; sat?: number; act?: number; state?: string; campusSizePreference?: 'small' | 'medium' | 'large'; costSensitivity?: 'high' | 'medium' | 'low'; targetMajor?: string }, schoolData?: { positionNeeds?: string[]; coachInterest?: 'low' | 'medium' | 'high'; campusSize?: number; avgGpa?: number; avgSat?: number; avgAct?: number; offeredMajors?: string[]; rosterDepth?: number; yearsToGraduate?: number; scholarshipAvailability?: 'high' | 'medium' | 'low'; state?: string; costOfAttendance?: number; walkOnHistory?: boolean; majorStrengthRating?: number }) => Promise<FitScoreResult>
  recalculateAllFitScores: (schools: School[]) => Promise<Map<string, FitScoreResult>>
  ...
  async function calculateSchoolFitScore(
    ...
    try {
      ...
      const personalFit = athleteData || schoolData
        ? calculatePersonalFit(
      ...
      const message = err instanceof Error ? err.message : 'Failed to calculate fit score'
   ...
   */
  async function recalculateAllFitScores(schools: School[]): Promise<Map<string, FitScoreResult>> {
    state.value.loading = true
    state.value.error = null
    ...
    try {
      ...
      for (const school of schools) {
        ...
        if ('fit_score' in school && school.fit_score !== null && school.fit_score !== undefined) {
          const fitData = 'fit_score_data' in school ? school.fit_score_data : null
          const fitScoreData = (fitData && typeof fitData === 'object' && 'athleticFit' in fitData)
      ...
      const message = err instanceof Error ? err.message : 'Failed to recalculate fit scores'
      state.value.error = message
   ...
   */
  async function getPortfolioHealth(schools: School[]): Promise<PortfolioHealth> {
    try {
      ...
      const schoolsWithFitScores = schools.map(school => {
        ...
        const fitScore = cached?.score || ('fit_score' in school && typeof school.fit_score === 'number' ? school.fit_score : 0)
        const fitTier = cached?.tier || ('fit_tier' in school && typeof school.fit_tier === 'string' ? school.fit_tier as FitTier : undefined)
        return {
          ...school,
          fit_score: fitScore,
          fit_tier: fitTier,
