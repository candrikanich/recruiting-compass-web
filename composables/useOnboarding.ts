...
export const useOnboarding = () => {
   ...
   */
  const calculateStartingPhase = (assessment: OnboardingAssessment): 'freshman' | 'sophomore' | 'junior' => {
  ...
  const getTasksToComplete = (assessment: OnboardingAssessment): string[] => {
    ...
    if (assessment.hasRegisteredEligibility) {
      ONBOARDING_TASK_MAPPING.hasRegisteredEligibility.forEach(t => tasks.add(t))
    }
   ...
   */
  const completeOnboarding = async (assessment: OnboardingAssessment): Promise<OnboardingResult> => {
    loading.value = true
    error.value = null
  ...
  const checkOnboardingStatus = async (userId?: string): Promise<boolean> => {
    try {
      ...
      const isComplete = data?.phase_milestone_data?.onboarding_complete === true
