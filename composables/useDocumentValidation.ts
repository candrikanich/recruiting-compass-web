...
const ALLOWED_MIME_TYPES = {
  ...
  'questionnaire': ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'],
  'stats_sheet': ['text/csv', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'],
}
 ...
 */
export const validateFile = (file: File, documentType: string): { valid: boolean; error?: string } => {
  ...
  const isValidType = allowedTypes.includes(file.type) || allowedExts.includes(fileExtension)
  if (!isValidType) {
    return { valid: false, error: `File type not allowed. Accepted: ${allowedExts.join(', ')}` }
  }
