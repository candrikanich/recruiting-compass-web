...
export const useTemplateUnlock = (): {
  ...
  checkTemplateUnlocked: (template: CommunicationTemplate) => Promise<{ unlocked: boolean; missingConditions: UnlockCondition[]; progressPercent: number }>
  getTemplatesWithUnlockStatus: (templates: CommunicationTemplate[]) => Promise<TemplateWithUnlockStatus[]>
  renderTemplate: (template: CommunicationTemplate, additionalVariables?: Record<string, string>) => Promise<{ subject: string; body: string }>
  filterPredefined: (templates: CommunicationTemplate[]) => CommunicationTemplate[]
} => {
   ...
   */
  const checkUnlockCondition = async (condition: UnlockCondition): Promise<boolean> => {
   ...
   */
  const checkTemplateUnlocked = async (template: CommunicationTemplate): Promise<{
    unlocked: boolean
    missingConditions: UnlockCondition[]
    progressPercent: number
  }> => {
  ...
  const getTemplatesWithUnlockStatus = async (
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to check template unlock status'
   ...
   */
  const filterPredefined = (templates: CommunicationTemplate[]): CommunicationTemplate[] => {
