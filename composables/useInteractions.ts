...
export const useInteractions = (): {
  ...
  fetchInteractions: (filters?: { schoolId?: string; coachId?: string; type?: string; direction?: string; sentiment?: string; startDate?: string; endDate?: string }) => Promise<void>
  getInteraction: (id: string) => Promise<Interaction | null>
  createInteraction: (interactionData: Omit<Interaction, 'id' | 'created_at'>, files?: File[]) => Promise<Interaction>
  updateInteraction: (id: string, updates: Partial<Interaction>) => Promise<Interaction>
  deleteInteraction: (id: string) => Promise<void>
  uploadAttachments: (files: File[], interactionId: string) => Promise<string[]>
  exportToCSV: () => string
  downloadCSV: () => void
} => {
  ...
  const fetchInteractions = async (filters?: { schoolId?: string; coachId?: string; type?: string; direction?: string; sentiment?: string; startDate?: string; endDate?: string }) => {
    loading.value = true
    error.value = null
  ...
  const validateAttachmentFile = (file: File): void => {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      throw new Error(`File type ${file.type} not allowed. Accepted: PDF, images, Word docs, Excel, text files`)
    }
  ...
  const uploadAttachments = async (files: File[], interactionId: string): Promise<string[]> => {
  ...
  const createInteraction = async (interactionData: Omit<Interaction, 'id' | 'created_at'>, files?: File[]) => {
