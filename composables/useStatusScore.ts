...
export const useStatusScore = (): {
  ...
  detailedBreakdown: ComputedRef<Array<{ label: string; value: number; weight: number; status: string }>>
   ...
   */
   const recalculateStatus = async (): Promise<AthleteAPI.RecalculateStatusResponse> => {
    loading.value = true
    error.value = null
    ...
    return [
      {
        ...
        weight: 35,
        status: scoreBreakdown.value.taskCompletionRate >= 70 ? 'good' : 'needs-work',
      },
      {
        label: 'Interaction Frequency',
        value: Math.round(scoreBreakdown.value.interactionFrequencyScore),
        weight: 25,
        status: scoreBreakdown.value.interactionFrequencyScore >= 70 ? 'good' : 'needs-work',
      },
      {
        label: 'Coach Interest',
        value: Math.round(scoreBreakdown.value.coachInterestScore),
        weight: 25,
        status: scoreBreakdown.value.coachInterestScore >= 60 ? 'good' : 'needs-work',
      },
      {
        label: 'Academic Standing',
        value: Math.round(scoreBreakdown.value.academicStandingScore),
        weight: 15,
        status: scoreBreakdown.value.academicStandingScore >= 60 ? 'good' : 'needs-work',
      },
    ]
  })
