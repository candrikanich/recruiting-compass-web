...
export const useSchoolMatching = () => {
   ...
   */
  const evaluatePreference = (school: School, pref: SchoolPreference): boolean => {
    switch (pref.type) {
      ...
      case 'conference_type':
        ...
        return evaluateConferenceType(school.conference, pref.value as string[])
  ...
  const evaluateDistance = (school: School, maxMiles: number): boolean => {
    ...
    const academicInfo = school.academic_info as { latitude?: number; longitude?: number } | null
  ...
  const getAcademicRating = (school: School): number => {
    const academicInfo = school.academic_info as { admission_rate?: number } | null
    ...
    if (rate < 0.30) return 4 // Excellent
    if (rate < 0.50) return 3 // Very Good
    if (rate < 0.70) return 2 // Good
    return 1 // Basic
  }
   ...
   */
  const evaluateConferenceType = (conference: string, preferredTypes: string[]): boolean => {
    const power4 = ['SEC', 'Big Ten', 'Big 12', 'ACC']
    const group5 = ['AAC', 'Mountain West', 'Sun Belt', 'MAC', 'Conference USA']

    const confUpper = conference.toUpperCase()

    for (const type of preferredTypes) {
      if (type === 'Power 4' && power4.some((c) => confUpper.includes(c.toUpperCase()))) {
        return true
      }
      if (type === 'Group of 5' && group5.some((c) => confUpper.includes(c.toUpperCase()))) {
        return true
      }
   ...
   */
  const evaluateSchoolSize = (school: School, sizeCategory: string): boolean => {
  ...
  const evaluateRegion = (school: School, preferredRegions: string[]): boolean => {
    ...
    for (const region of preferredRegions) {
      ...
      if (states.some((st) => location.includes(st) || location.includes(getStateName(st)))) {
        return true
      }
    }
    return false
   ...
   */
  const evaluateState = (school: School, preferredStates: string[]): boolean => {
    if (!school.location) return false
    const location = school.location.toLowerCase()
    return preferredStates.some(
      (st) => location.includes(st.toLowerCase()) || location.includes(getStateName(st.toLowerCase()))
    )
  }
