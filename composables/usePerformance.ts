...
export const usePerformance = (): {
  ...
  fetchMetrics: (filters?: { metricType?: string; eventId?: string; startDate?: string; endDate?: string }) => Promise<void>
  createMetric: (metricData: Omit<PerformanceMetric, 'id' | 'created_at' | 'updated_at'>) => Promise<PerformanceMetric>
  updateMetric: (id: string, updates: Partial<PerformanceMetric>) => Promise<PerformanceMetric>
  ...
  const fetchMetrics = async (filters?: { metricType?: string; eventId?: string; startDate?: string; endDate?: string }) => {
    ...
    try {
      let query = supabase.from('performance_metrics').select('*').eq('user_id', userStore.user.id)
  ...
  const createMetric = async (metricData: Omit<PerformanceMetric, 'id' | 'created_at' | 'updated_at'>) => {
  ...
  const updateMetric = async (id: string, updates: Partial<PerformanceMetric>) => {
    if (!userStore.user) throw new Error('User not authenticated')
