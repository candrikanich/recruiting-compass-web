...
const schoolNameMatches = (schoolName: string, knownName: string): boolean => {
  ...
  if (normalizedSchool.length > 8 && normalizedKnown.length > 8) {
    if (normalizedSchool.includes(normalizedKnown) || normalizedKnown.includes(normalizedSchool)) {
      return true
    }
  }
  ...
  if (lenDiff <= 3) {
    const maxDistance = Math.max(normalizedSchool.length, normalizedKnown.length) > 6 ? 2 : 1
...
export const useNcaaLookup = () => {
   ...
   */
  const lookupSchool = async (schoolName: string): Promise<NcaaLookupResult | null> => {
