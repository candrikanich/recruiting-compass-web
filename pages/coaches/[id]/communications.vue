    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p class="text-2xl font-bold text-gray-900">{{ filteredInteractions.length }}</p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
        ...
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p class="text-2xl font-bold text-purple-600">{{ avgResponseTime }}h</p>
        </div>
      </div>
      ...
      <div v-if="!loading && filteredInteractions.length > 0" class="space-y-4">
        <div
          ...
          :class="[
            ...
            interaction.direction === 'outbound' ? 'border-l-blue-500' : 'border-l-green-500',
          ]"
        >
          <!-- Message Header -->
          <div class="flex items-start justify-between mb-3">
            <div>
              <div class="flex items-center gap-2 mb-1">
                ...
                <span v-if="interaction.sentiment" :class="getSentimentClass(interaction.sentiment)">
                  {{ formatSentiment(interaction.sentiment) }}
                </span>
              </div>
              <p class="text-sm text-gray-500">{{ formatDate(interaction.occurred_at) }}</p>
            </div>
          </div>
          ...
          <!-- Content -->
          <div v-if="interaction.content" class="mb-4 text-gray-700 whitespace-pre-wrap break-words">
            {{ interaction.content }}
          </div>

          <!-- Attachments -->
          <div v-if="interaction.attachments && interaction.attachments.length > 0" class="mt-4 pt-4 border-t border-gray-200">
      ...
      <!-- Empty State -->
      <div v-if="!loading && filteredInteractions.length === 0" class="bg-white rounded-lg shadow p-12 text-center">
...
const filteredInteractions = computed(() => {
  ...
  if (selectedDateRange.value) {
    ...
    filtered = filtered.filter((i) => new Date(i.occurred_at || '') > cutoffDate)
  }
...
const outboundCount = computed(() => filteredInteractions.value.filter((i) => i.direction === 'outbound').length)
const inboundCount = computed(() => filteredInteractions.value.filter((i) => i.direction === 'inbound').length)
...
const avgResponseTime = computed(() => {
  ...
  for (let i = 0; i < filteredInteractions.value.length - 1; i++) {
    ...
    if (current.direction === 'outbound') {
      const nextInbound = filteredInteractions.value.slice(i + 1).find((x) => x.direction === 'inbound')
      if (nextInbound) {
        const responseMs = new Date(nextInbound.occurred_at || '').getTime() - new Date(current.occurred_at || '').getTime()
  ...
  return responseCount > 0 ? Math.round((totalTime / responseCount / (1000 * 60 * 60)) * 10) / 10 : 0
})
...
const getSentimentClass = (sentiment: string): string => {
  const classes: Record<string, string> = {
    very_positive: 'px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-800',
    positive: 'px-2 py-1 rounded text-xs font-semibold bg-blue-100 text-blue-800',
    neutral: 'px-2 py-1 rounded text-xs font-semibold bg-gray-100 text-gray-800',
