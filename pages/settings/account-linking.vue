  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
    ...
    <div class="bg-white border-b border-slate-200">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 py-4">
        ...
        <h1 class="text-2xl font-semibold text-slate-900">Family Account Linking</h1>
        <p class="text-slate-600">Share recruiting data with a parent or player account</p>
      </div>
    </div>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        ...
        <div v-else class="text-center py-8 text-gray-500">
          ...
          <p class="text-xs mt-1">Send an invitation below to link with a parent or player</p>
        </div>
      </div>

      <!-- Pending Invitations Section -->
      <div v-if="pendingInvitations.length > 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        <h2 class="text-xl font-bold text-gray-900 mb-4">⏳ Pending Invitations</h2>
        ...
        <form @submit.prevent="handleSendInvitation" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              {{ userStore.user?.role === 'parent' ? 'Player' : 'Parent' }} Email Address
          ...
          <div v-if="sendError" class="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700 text-sm">
            {{ sendError }}
          </div>
        ...
        <div class="mt-6 pt-6 border-t border-gray-200">
          ...
          <ul class="text-sm text-gray-600 mt-3 space-y-2">
            <li class="flex items-start gap-2">
              ...
              <span>Enter the email address of the parent or player you want to link with</span>
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-600 font-bold">2.</span>
              <span>They'll receive a notification to accept or decline the invitation</span>
            </li>
            <li class="flex items-start gap-2">
              <span class="text-blue-600 font-bold">3.</span>
              <span>Once accepted, you'll both see the same schools, coaches, events, and interactions</span>
            </li>
            <li class="flex items-start gap-2">
...
const { linkedAccounts, pendingInvitations, loading, error, fetchAccountLinks, sendInvitation } =
  useAccountLinks()
