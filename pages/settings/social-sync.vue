  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
    ...
    <div class="bg-white border-b border-slate-200">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 py-4">
        ...
        <p class="text-slate-600">Configure automatic social media monitoring</p>
      </div>
    </div>
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
          <div class="bg-gray-50 rounded-lg p-4">
            ...
            <p class="text-lg font-semibold text-gray-900">{{ lastSyncDisplay }}</p>
          </div>
          <div class="bg-gray-50 rounded-lg p-4">
        ...
        <div v-if="syncMessage" :class="['mt-4 p-4 rounded-lg', syncSuccess ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800']">
          {{ syncMessage }}
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        ...
        <div class="mb-4">
          ...
          <p class="text-sm text-gray-500 mt-1 ml-8">
            When enabled, posts from your schools and coaches will be automatically synced every 6 hours
          </p>
        </div>

        <div v-if="autoSyncEnabled" class="mt-4 p-4 bg-blue-50 rounded-lg">
          <p class="text-sm text-blue-800">
            <strong>Next scheduled sync:</strong> Automatic sync runs via GitLab CI schedules every 6 hours.
            Configure schedules in your GitLab project settings.
          </p>
        </div>
      </div>
      ...
      <!-- Tracked Accounts -->
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Tracked Accounts</h2>

        <div v-if="loading" class="text-center py-4">
          <p class="text-gray-600">Loading accounts...</p>
        </div>

        <div v-else-if="trackedAccounts.length === 0" class="text-center py-8 text-gray-500">
          <p>No social accounts tracked yet.</p>
          <p class="text-sm mt-2">Add Twitter or Instagram handles to schools and coaches to start tracking.</p>
        </div>

        <div v-else class="space-y-4">
          <!-- Twitter Accounts -->
          <div v-if="twitterAccounts.length > 0">
            <h3 class="text-sm font-semibold text-gray-700 mb-2">Twitter/X Accounts ({{ twitterAccounts.length }})</h3>
            <div class="flex flex-wrap gap-2">
              ...
              >
                @{{ account.handle }}
                <span class="text-blue-600 text-xs ml-1">({{ account.source }})</span>
              </span>
            </div>
          </div>
          ...
          <div v-if="instagramAccounts.length > 0" class="mt-4">
            <h3 class="text-sm font-semibold text-gray-700 mb-2">Instagram Accounts ({{ instagramAccounts.length }})</h3>
            <div class="flex flex-wrap gap-2">
              <span
              ...
              >
                @{{ account.handle }}
                <span class="text-pink-600 text-xs ml-1">({{ account.source }})</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <div class="space-y-4">
          <label class="flex items-center gap-3 cursor-pointer">
            ...
            />
            <div>
              <span class="text-gray-700 font-medium">Recruiting post alerts</span>
              <p class="text-sm text-gray-500">Get notified when new recruiting-related posts are found</p>
            </div>
          </label>
          ...
          <label class="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">Alert when posts mention your tracked positions (e.g., "looking for a SS")</p>
            </div>
          </label>
        </div>
...
const twitterAccounts = computed(() => trackedAccounts.value.filter(a => a.platform === 'twitter'))
const instagramAccounts = computed(() => trackedAccounts.value.filter(a => a.platform === 'instagram'))
...
const loadSettings = async () => {
  ...
  try {
    ...
    if (data?.social_sync_settings) {
      ...
      notifyOnRecruitingPosts.value = data.social_sync_settings.notifyOnRecruitingPosts ?? false
      notifyOnMentions.value = data.social_sync_settings.notifyOnMentions ?? false
