  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
    ...
    <div class="bg-white border-b border-slate-200">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 py-4">
        ...
        <h1 class="text-2xl font-semibold text-slate-900">School Preferences</h1>
        <p class="text-slate-600">Set your criteria for finding the ideal schools</p>
      </div>
    </div>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div v-else class="space-y-8">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h2 class="text-xl font-bold text-gray-900 mb-4">Start from a Template</h2>
          <p class="text-sm text-gray-600 mb-4">Choose a template to quickly set up your preferences</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div class="flex items-center justify-between mb-6">
            <div>
              ...
              <p class="text-sm text-gray-600">Drag to reorder. Top items are highest priority.</p>
            </div>
            <button
          ...
          <!-- Empty State -->
          <div v-if="preferences.length === 0" class="text-center py-12 text-gray-500">
            <p>No preferences set yet.</p>
            <p class="text-sm mt-2">Choose a template above or add preferences manually.</p>
          </div>
            ...
            >
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2">
                  <span class="font-medium text-gray-900">{{ getPreferenceLabel(pref) }}</span>
                  <span
                ...
                <p class="text-sm text-gray-600">{{ getPreferenceValue(pref) }}</p>
              </div>
        ...
        >
          <div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="p-6">
              <h3 class="text-xl font-bold text-gray-900 mb-4">Add Preference</h3>
              ...
              <div class="mb-4">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <button
                    ...
                    :key="cat.value"
                    @click="newPreference.category = cat.value as PreferenceCategory"
                    :class="[
              ...
              <!-- Type Selection -->
              <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Preference Type</label>
                <select
              ...
              <div v-if="newPreference.type" class="mb-4">
                ...
                <!-- Distance Input -->
                <div v-if="newPreference.type === 'max_distance_miles'" class="flex items-center gap-2">
                  <input
                ...
                <div v-else-if="newPreference.type === 'division'" class="flex flex-wrap gap-2">
                  <button
                ...
                <div v-else-if="newPreference.type === 'conference_type'" class="flex flex-wrap gap-2">
                  <button
                ...
                <!-- Boolean -->
                <div v-else-if="newPreference.type === 'scholarship_required'" class="flex gap-4">
                  <label class="flex items-center gap-2">
                    <input type="radio" :value="true" v-model="newPreference.value" />
                    <span>Yes, required</span>
                  </label>
                  <label class="flex items-center gap-2">
                    <input type="radio" :value="false" v-model="newPreference.value" />
                    <span>Not required</span>
                  </label>
                </div>

                <!-- Regions -->
                <div v-else-if="newPreference.type === 'preferred_regions'" class="flex flex-wrap gap-2">
                  <button
              ...
              <div class="mb-6">
                <label class="flex items-center gap-3 cursor-pointer">
                  ...
                  <span class="text-sm text-gray-700">
                    Mark as dealbreaker (schools without this will show a warning)
                  </span>
                </label>
              </div>
              ...
              <div class="flex justify-end gap-3">
                ...
                <button
                  @click="addPreference"
                  :disabled="!newPreference.type || newPreference.value === undefined"
...
const { schoolPreferences, loading, error, fetchPreferences, updateSchoolPreferences } = useUserPreferences()
...
const CONFERENCE_TYPES = ['Power 4', 'Group of 5', 'Mid-Major', 'Small Conference']
const REGIONS = ['Northeast', 'Southeast', 'Midwest', 'Southwest', 'West Coast', 'Pacific Northwest']
...
const TEMPLATES = [
  {
    ...
    preferences: [
      { category: 'program', type: 'division', value: ['D1'], is_dealbreaker: true },
      { category: 'program', type: 'conference_type', value: ['Power 4'], is_dealbreaker: false },
      { category: 'program', type: 'scholarship_required', value: true, is_dealbreaker: false },
    ],
  },
  {
    ...
    preferences: [
      { category: 'academic', type: 'min_academic_rating', value: 4, is_dealbreaker: true },
      { category: 'academic', type: 'school_size', value: 'medium', is_dealbreaker: false },
      { category: 'program', type: 'division', value: ['D1', 'D3'], is_dealbreaker: false },
    ],
  },
  {
    id: 'close_to_home',
    ...
    preferences: [
      { category: 'location', type: 'max_distance_miles', value: 300, is_dealbreaker: true },
    ],
  },
  {
    id: 'best_fit',
    ...
    preferences: [
      { category: 'location', type: 'max_distance_miles', value: 500, is_dealbreaker: false },
      { category: 'academic', type: 'min_academic_rating', value: 3, is_dealbreaker: false },
      { category: 'program', type: 'division', value: ['D1', 'D2', 'D3'], is_dealbreaker: false },
    ],
  },
]
...
const getPreferenceLabel = (pref: SchoolPreference) => {
  const types = PREFERENCE_TYPES[pref.category as keyof typeof PREFERENCE_TYPES] || []
...
const toggleDealbreaker = (index: number) => {
  preferences.value[index].is_dealbreaker = !preferences.value[index].is_dealbreaker
}
