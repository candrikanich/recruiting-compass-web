...
<template>
  <section>
    ...
    >
      ...
      <span class="font-semibold">{{ detail?.account.email ?? "…" }}</span>'s data.
    </div>
    ...
    <template v-else-if="detail">
      ...
      <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <AdminStatTile label="Schools" :value="detail.recruiting.counts.schools" />
        <AdminStatTile label="Coaches" :value="detail.recruiting.counts.coaches" />
        <AdminStatTile label="Interactions" :value="detail.recruiting.counts.interactions" />
        <AdminStatTile label="Offers" :value="detail.recruiting.counts.offers" />
        <AdminStatTile label="Events" :value="detail.recruiting.counts.events" />
        <AdminStatTile label="Messages" :value="detail.recruiting.counts.messages" />
      </div>
      ...
      <dl class="mb-6 grid grid-cols-2 gap-x-6 gap-y-1 text-sm sm:grid-cols-3">
        ...
        <div>
          ...
          <dd class="text-brand-slate-800">{{ detail.account.current_phase ?? "—" }}</dd>
        </div>
        <div>
          <dt class="text-brand-slate-500">Grad year</dt>
          <dd class="text-brand-slate-800">{{ detail.account.graduation_year ?? "—" }}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd class="text-brand-slate-800">{{ new Date(detail.account.created_at).toLocaleDateString() }}</dd>
        </div>
      </dl>
