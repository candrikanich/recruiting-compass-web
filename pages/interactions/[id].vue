    ...
    <div v-if="interaction" class="space-y-6">
      ...
      <div class="flex items-start justify-between">
        <div>
          ...
          <p class="text-gray-600 mt-2">{{ formatDate(interaction.occurred_at) }}</p>
        </div>
        <div class="flex gap-2">
          <button @click="exportInteraction" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            📤 Export
          </button>
          <button @click="deleteInteraction" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">
            🗑️ Delete
          </button>
        </div>
      </div>
      ...
      <div class="flex gap-2">
        ...
        <span :class="[
          ...
          interaction.direction === 'inbound' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800'
        ]">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p v-if="school" class="text-gray-700">
            <NuxtLink :to="`/schools/${school.id}`" class="text-blue-600 hover:underline">
              {{ school.name }}
            </NuxtLink>
          </p>
        ...
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p v-if="coach" class="text-gray-700">
            <NuxtLink :to="`/coaches/${coach.id}`" class="text-blue-600 hover:underline">
        ...
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p v-if="event" class="text-gray-700">
            <NuxtLink :to="`/events/${event.id}`" class="text-blue-600 hover:underline">
              {{ event.name }}
            </NuxtLink>
          </p>
          <p v-else class="text-gray-500">—</p>
      ...
      <!-- Attachments -->
      <div v-if="interaction.attachments && interaction.attachments.length > 0" class="bg-white rounded-lg shadow p-6">
        <h2 class="text-xl font-bold mb-4">Attachments ({{ interaction.attachments.length }})</h2>
        <div class="grid grid-cols-2 gap-4">
          <a
          ...
          >
            <p class="text-sm font-medium text-blue-600 break-all">📎 {{ getFilename(url) }}</p>
          </a>
        </div>
      </div>
      ...
      <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-600 space-y-2">
        <p><span class="font-semibold">Created:</span> {{ formatDate(interaction.created_at) }}</p>
      </div>
    </div>
...
const school = computed(() => {
  ...
  return schools.value?.find(s => s.id === interaction.value?.school_id) ?? null
})

const coach = computed(() => {
  if (!interaction.value?.coach_id) return null
  return coaches.value?.find(c => c.id === interaction.value?.coach_id) ?? null
})

const event = computed(() => {
  if (!interaction.value?.event_id) return null
  return events.value?.find(e => e.id === interaction.value?.event_id) ?? null
})
...
const exportInteraction = () => {
  ...
  const csv = [
    ...
    ['Coach', coach.value ? `${coach.value.first_name} ${coach.value.last_name}` : 'N/A'],
