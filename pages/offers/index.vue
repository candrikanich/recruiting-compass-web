<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p class="text-slate-600">{{ filteredOffers.length }} offer{{ filteredOffers.length !== 1 ? 's' : '' }} found</p>
          </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-2xl font-bold text-slate-900">{{ acceptedOffers.length }}</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-2xl font-bold text-slate-900">{{ pendingOffers.length }}</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-2xl font-bold text-slate-900">{{ declinedOffers.length }}</p>
      ...
      <!-- Add Offer Form -->
      <div v-if="showAddForm" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
        ...
        <form @submit.prevent="handleAddOffer" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Scholarship %</label>
              <input
            ...
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Offer Date *</label>
              <input
      ...
      <!-- Loading State -->
      <div v-if="loading && offers.length === 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-4"></div>
        <p class="text-slate-600">Loading offers...</p>
      </div>

      <!-- Empty State -->
      <div v-else-if="offers.length === 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
        <GiftIcon class="w-12 h-12 text-slate-300 mx-auto mb-4" />
        <p class="text-slate-900 font-medium mb-2">No offers logged yet</p>
        <p class="text-sm text-slate-500">Log scholarship offers as you receive them</p>
      </div>

      <!-- No Results State -->
      <div v-else-if="filteredOffers.length === 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
        <MagnifyingGlassIcon class="w-12 h-12 text-slate-300 mx-auto mb-4" />
        <p class="text-slate-900 font-medium mb-2">No offers match your filters</p>
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :class="[
            isOfferSelected(offer.id) ? 'border-blue-400 ring-2 ring-blue-100' : 'border-slate-200',
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-start justify-between gap-4">
                  <div>
                    <div class="flex items-center gap-2 flex-wrap mb-1">
                      <h3 class="font-semibold text-slate-900">{{ getSchoolName(offer.school_id) }}</h3>
                      <span
                    ...
                    <p class="text-sm text-slate-500">{{ getOfferTypeLabel(offer.offer_type) }}</p>
                  </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                  <div v-if="offer.scholarship_percentage">
                    ...
                    <p class="text-lg font-bold text-slate-900">{{ offer.scholarship_percentage }}%</p>
                  </div>
                  <div v-if="offer.scholarship_amount">
                    <p class="text-xs text-slate-500">Amount</p>
                    <p class="text-lg font-bold text-slate-900">${{ formatAmount(offer.scholarship_amount) }}</p>
                  </div>
                  <div>
                    <p class="text-xs text-slate-500">Offered</p>
                    <p class="text-sm font-medium text-slate-700">{{ formatDate(offer.offer_date) }}</p>
                  </div>
                  <div v-if="offer.deadline_date">
                    <p class="text-xs text-slate-500">Deadline</p>
                    <p class="text-sm font-medium" :class="getDeadlineClass(offer)">
                      {{ formatDate(offer.deadline_date) }}
                      <span v-if="daysUntilDeadline(offer) !== null" class="text-xs">
                ...
                <!-- Notes -->
                <p v-if="offer.notes" class="text-sm text-slate-600 mt-3 line-clamp-2">
                  {{ offer.notes }}
                </p>
              </div>
            </div>
...
const { offers, acceptedOffers, pendingOffers, declinedOffers, loading, fetchOffers, createOffer, deleteOffer: deleteOfferAPI, daysUntilDeadline } = useOffers()
...
const handleAddOffer = async () => {
  try {
    await createOffer({
      ...
      offer_type: newOffer.offer_type as 'full_ride' | 'partial' | 'scholarship' | 'recruited_walk_on' | 'preferred_walk_on',
      ...
      status: newOffer.status as 'pending' | 'accepted' | 'declined' | 'expired',
