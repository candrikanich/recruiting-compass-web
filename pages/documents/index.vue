    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-sm text-gray-500">{{ filteredDocuments.length }} of {{ documents.length }} total</span>
        </div>
        <p class="text-gray-600 mt-1">Manage videos, transcripts, and other recruiting documents</p>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white rounded-lg shadow p-4">
          ...
          <p class="text-2xl font-bold text-green-600">{{ sharedDocumentsCount }}</p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
        ...
        <template #filter>
          <div class="bg-white p-4 rounded-lg shadow">
            ...
            <div class="mt-6 pt-6 border-t">
              <label for="sort" class="text-sm font-medium text-gray-700 mb-2 block">Sort By</label>
              <select
                v-model="sortBy"
                id="sort"
      ...
      <div v-if="showUploadForm" class="bg-white rounded-lg shadow p-6 mb-8">
        ...
        <form @submit.prevent="handleUpload" class="space-y-6">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 mt-2">Allowed formats: {{ allowedFileTypes }}</p>
          </div>
          ...
          <div class="flex gap-4">
            <button
              type="submit"
              :disabled="loading || !newDoc.type || !newDoc.title || !selectedFile"
      ...
      <div v-else-if="documents.length === 0" class="bg-white rounded-lg shadow p-12 text-center">
        ...
        <p class="text-sm text-gray-500">Upload videos, transcripts, and other documents to share with coaches</p>
      </div>
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-600">
                    {{ getSchoolName(doc.school_id) }} • v{{ doc.version || 1 }} • {{ formatDate(doc.created_at) }}
                  </p>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-3">
              <span v-if="(doc.shared_with_schools || []).length > 0" class="text-xs px-2 py-1 bg-green-100 text-green-700 rounded">
...
const filteredDocuments = computed(() => {
  return documents.value.filter((doc: Document) => {
    ...
    if (sharedFilter !== undefined && sharedFilter !== null) {
      const isShared = doc.shared_with_schools && doc.shared_with_schools.length > 0
...
const sharedDocumentsCount = computed(() => {
  return documents.value.filter((doc: Document) => doc.shared_with_schools && doc.shared_with_schools.length > 0).length
})
...
const mostCommonType = computed(() => {
  ...
  const mostCommon = Object.entries(typeCounts).sort((a, b) => (b[1] as number) - (a[1] as number))[0] as [string, number] | undefined
