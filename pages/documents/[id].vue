<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Back Button -->
      <div class="mb-6">
        <NuxtLink to="/documents" class="text-blue-600 hover:text-blue-700 font-semibold">
          ← Back to Documents
        </NuxtLink>
      </div>
      ...
      <!-- Document Not Found -->
      <div v-else-if="!document" class="bg-white rounded-lg shadow p-12 text-center">
      ...
      <div v-else class="space-y-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <div class="flex items-start justify-between mb-6">
            <div class="flex-1">
              ...
              <p v-if="document.description" class="text-gray-600">{{ document.description }}</p>
            </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-gray-200">
            ...
            <div>
              ...
              <p class="font-semibold text-gray-900">{{ formatDate(document.created_at) }}</p>
            </div>
            <div>
              <p class="text-gray-600 text-sm mb-1">File Type</p>
              <p class="font-semibold text-gray-900">{{ document.file_type || 'Unknown' }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <!-- Image -->
          <img v-else-if="isImage" :src="document.file_url" class="max-w-full rounded-lg" />

          <!-- PDF -->
          <iframe v-else-if="isPDF" :src="document.file_url" class="w-full h-96 rounded-lg border border-gray-300" />

          <!-- Download for other types -->
          <div v-else class="text-center py-12">
            <p class="text-gray-600 mb-4">File preview not available for this file type</p>
            <a
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div v-if="documentVersions.length > 0" class="space-y-2">
            <div v-for="version in documentVersions" :key="version.id" class="flex items-center justify-between py-3 border-b border-gray-200">
              <div class="flex-1">
                <div class="flex items-center gap-3">
                  <span class="font-semibold text-gray-900">Version {{ version.version }}</span>
                  <span v-if="version.is_current" class="inline-block px-2 py-1 text-xs font-semibold bg-blue-100 text-blue-800 rounded">
                    Current
                  </span>
                </div>
                <p class="text-sm text-gray-500 mt-1">{{ formatDate(version.created_at) }}</p>
              </div>
              <div class="flex gap-2">
                <a
      ...
      <div v-if="showShareModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-4 max-h-96 overflow-y-auto">
          <h3 class="text-lg font-bold text-gray-900 mb-4">Share Document</h3>

          <!-- Currently Shared Schools -->
          <div v-if="(document?.shared_with_schools || []).length > 0" class="mb-6">
            <h4 class="text-sm font-semibold text-gray-700 mb-3">Shared With</h4>
            <div class="space-y-2">
              <div
              ...
              >
                <span class="text-sm text-gray-900">{{ getSchoolNameById(schoolId) }}</span>
                <button
          ...
          <!-- School Selector -->
          <div class="mb-6">
            <h4 class="text-sm font-semibold text-gray-700 mb-3">Add Schools</h4>
...
const document = computed(() => documents.value.find((d: Document) => d.id === documentId.value))

const schoolName = computed(() => {
  if (!document.value || !document.value.school_id) return 'Not specified'
  return schools.value.find((s: School) => s.id === document.value!.school_id)?.name || 'Unknown School'
})

const isVideo = computed(() => {
  if (!document.value) return false
  return document.value.type === 'highlight_video' || document.value.file_type?.includes('video')
})
...
const availableSchools = computed(() => {
  return schools.value.filter((s) => !(document.value?.shared_with_schools || []).includes(s.id))
})
...
const restoreVersion = async (versionId: string) => {
  if (confirm('Restore this version? The current version will be marked as archived.')) {
    try {
