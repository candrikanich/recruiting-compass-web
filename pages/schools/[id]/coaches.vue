    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-slate-200">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <!-- Sort -->
          <div>
            <label for="sortFilter" class="block text-sm font-medium text-slate-700 mb-2">
              Sort by
            </label>
            <select
              id="sortFilter"
        ...
        <div v-if="searchQuery || roleFilter || sortFilter !== 'name'" class="mt-4 flex justify-end">
          <button
            @click="clearFilters"
      ...
      <!-- Add Coach Form -->
      <div v-if="showAddForm" class="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-slate-200">
      ...
      <div v-if="coaches.length > 0" class="mb-6">
        <p class="text-sm text-slate-600">
          Showing <span class="font-semibold">{{ filteredCoaches.length }}</span>
          of <span class="font-semibold">{{ coaches.length }}</span>
      ...
      <!-- Empty State -->
      <div v-if="!loading && coaches.length === 0" class="bg-white rounded-2xl shadow-lg p-8 text-center border border-slate-200">
        <p class="text-slate-600 mb-4">No coaches added yet</p>
      </div>

      <!-- No Results State -->
      <div v-if="!loading && coaches.length > 0 && filteredCoaches.length === 0" class="bg-white rounded-2xl shadow-lg p-8 text-center border border-slate-200">
...
const { coaches, loading, error, fetchCoaches, createCoach, deleteCoach: deleteCoachAPI, getCoach } = useCoaches()
...
const handleSchoolCoachInteractionLogged = async (interactionData: any) => {
  try {
    ...
    localError.value = err instanceof Error ? err.message : 'Failed to log interaction'
  }
}
