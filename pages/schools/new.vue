<template>
  <div class="min-h-screen bg-slate-50">
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-6">
        <NuxtLink to="/schools" class="text-indigo-600 hover:text-indigo-700 font-semibold">
          ← Back to Schools
        </NuxtLink>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-8 py-6">
          ...
          <p class="text-blue-100 text-sm">Add a school to your recruiting list</p>
        </div>
        ...
        <div class="p-8">
          ...
          <div class="mb-6 bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
            <label class="flex items-center gap-3 cursor-pointer group">
              ...
              />
              <span class="text-slate-700 font-medium group-hover:text-blue-600 transition-colors">Search college database</span>
            </label>
          </div>

          <!-- Selected College Confirmation -->
          <div v-if="selectedCollege" class="mb-6 p-4 bg-green-50 border-2 border-green-200 rounded-xl">
            <div class="flex justify-between items-start gap-4">
              <div class="flex-1">
                <p class="text-sm text-green-800">
                  <strong>Selected:</strong> {{ selectedCollege.name }}
                </p>
                <p class="text-xs text-green-700 mt-1">{{ selectedCollege.location }}</p>
                <div v-if="collegeDataLoading" class="mt-2 text-xs text-blue-600">
                  Fetching college data...
                </div>
                <div v-else-if="collegeScorecardFetched" class="mt-2 text-xs text-green-700 flex items-center gap-1">
        ...
        <div class="mb-6">
          <label for="name" class="block text-sm font-medium text-gray-700 mb-1">
            ...
            <span v-if="autoFilledFields.name" class="text-xs text-blue-600 font-normal">(auto-filled)</span>
          </label>
        ...
        <div v-if="collegeScorecardData" class="border-t border-slate-200 pt-6 mt-6">
...
import { useCollegeData, type CollegeDataResult } from '~/composables/useCollegeData'
