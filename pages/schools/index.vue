  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p class="text-slate-600">{{ filteredSchools.length }} school{{ filteredSchools.length !== 1 ? 's' : '' }} found</p>
          </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input
                ...
                @input="handleFilterUpdate('name', ($event.target as HTMLInputElement).value)"
          ...
          <div>
            ...
            <select
              ...
              @change="handleFilterUpdate('division', ($event.target as HTMLSelectElement).value || null)"
          ...
          <div>
            ...
            <select
              ...
              @change="handleFilterUpdate('status', ($event.target as HTMLSelectElement).value || null)"
              class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
              <option value="">-- All --</option>
          ...
          <!-- Favorites -->
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Favorites Only</label>
            <select
              :value="(filterValues.value as any)?.is_favorite ? 'true' : ''"
              @change="handleFilterUpdate('is_favorite', ($event.target as HTMLSelectElement).value === 'true' || null)"
              class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
              <option value="">-- All --</option>
        ...
        <!-- Active Filters -->
        <div v-if="hasActiveFilters" class="flex items-center gap-2 mt-4 pt-4 border-t border-slate-200">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            >
              {{ value }}
              <button @click="handleRemoveFilter(key as string)" class="hover:text-blue-900">
                <XMarkIcon class="w-3 h-3" />
              </button>
            </span>
          </div>
          <button @click="clearFilters" class="text-sm text-slate-500 hover:text-slate-700 ml-2">
            Clear all
          </button>
        </div>
      </div>
      ...
      <!-- Empty State -->
      <div v-if="!loading && filteredSchools.length === 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
        <MagnifyingGlassIcon class="w-16 h-16 text-slate-300 mx-auto mb-4" />
        <h3 class="text-lg font-semibold text-slate-900 mb-2">No schools found</h3>
      ...
      <!-- Schools Grid -->
      <div v-if="!loading && filteredSchools.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
        ...
        >
          ...
          <div class="p-5">
            <div class="flex items-start gap-4 mb-4">
              <!-- Logo -->
              <SchoolLogo :school="school" size="lg" fetch-on-mount class="shadow-md rounded-lg" />

              <!-- Info -->
              <div class="flex-1 min-w-0">
                <h3 class="text-slate-900 font-semibold line-clamp-2 mb-1">{{ school.name }}</h3>
              ...
              <button
                ...
                :class="[
                  ...
                  school.is_favorite ? 'text-yellow-500' : 'text-slate-300 hover:text-yellow-400'
                ]"
              >
                <StarIcon :class="['w-5 h-5', school.is_favorite ? 'fill-yellow-500' : '']" />
              </button>
            </div>

            <!-- Badges -->
            <div class="flex flex-wrap gap-2 mb-4">
              <span v-if="school.division" class="px-2 py-0.5 text-xs font-medium rounded-full bg-blue-100 text-blue-700">
                {{ school.division }}
              </span>
              <span :class="getStatusBadgeClass(school.status)" class="px-2 py-0.5 text-xs font-medium rounded-full">
                {{ formatStatus(school.status) }}
              </span>
              <span
                v-if="getCarnegieSize(typeof school.academic_info?.student_size === 'string' ? parseInt(school.academic_info.student_size) : typeof school.academic_info?.student_size === 'number' ? school.academic_info.student_size : null)"
                :class="getSizeBadgeClass(getCarnegieSize(typeof school.academic_info?.student_size === 'string' ? parseInt(school.academic_info.student_size) : typeof school.academic_info?.student_size === 'number' ? school.academic_info.student_size : null))"
                class="px-2 py-0.5 text-xs font-medium rounded-full"
              >
                {{ getCarnegieSize(typeof school.academic_info?.student_size === 'string' ? parseInt(school.academic_info.student_size) : typeof school.academic_info?.student_size === 'number' ? school.academic_info.student_size : null) }}
              </span>
            </div>

            <!-- Conference/Notes -->
            <p v-if="school.conference" class="text-slate-600 text-sm mb-2">{{ school.conference }}</p>
            <p v-if="school.notes" class="text-slate-600 text-sm line-clamp-2">{{ school.notes }}</p>
          </div>
...
import { exportSchoolComparisonToCSV, generateSchoolComparisonPDF, type SchoolComparisonData } from '~/utils/exportUtils'
...
const { schools, loading, error, fetchSchools, toggleFavorite, deleteSchool } = useSchools()
const { fetchMultipleLogos } = useSchoolLogos()
const { calculateMatchScore } = useSchoolMatching()
const { fetchPreferences, schoolPreferences, homeLocation } = useUserPreferences()
...
const filterConfigs: FilterConfig[] = [
  { type: 'text', field: 'name', label: 'Search', placeholder: 'School name or location...' },
  {
    type: 'select',
    field: 'division',
    label: 'Division',
...
} = useUniversalFilter(schools as any, filterConfigs, { storageKey: 'schools-filters' })
...
const filteredSchools = computed(() => {
  ...
  // Apply custom search filter (searches name and location)
  const searchTerm = filterValues.value?.name as string
  if (searchTerm) {
...
const getExportData = (): SchoolComparisonData[] => {
  return filteredSchools.value.map((school) => {
    ...
    const interactionCount = allInteractions.value.filter((i) => i.school_id === school.id).length
    const coachCount = allCoaches.value.filter((c) => c.school_id === school.id).length

    let distance: number | null = null
    if (homeLocation.value?.latitude && homeLocation.value?.longitude && school.academic_info) {
