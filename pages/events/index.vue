<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
    ...
    <div class="bg-white border-b border-slate-200">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          <div class="flex items-center justify-between">
            <h1 data-testid="page-title" class="text-2xl font-bold text-slate-900">Events</h1>
            <p class="text-slate-600 mt-1">Track camps, showcases, visits, and games</p>
          </div>
            <NuxtLink
              to="/events/create"
    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      <!-- Empty State -->
      <div v-else-if="events.length === 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
      ...
      <template v-else>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="px-4 py-1 font-semibold text-slate-900 min-w-[160px] text-center">
                {{ monthDisplay }}
              </span>
              <button
                @click="nextMonth"
          ...
          <div class="grid grid-cols-7 gap-1">
            ...
            <div
              ...
              :class="[
                'p-2 rounded-lg text-sm font-medium transition min-h-[48px] flex flex-col items-center justify-center',
                {
                  ...
                  'text-slate-700 hover:bg-slate-50': isCurrentMonth(date) && !hasEvent(date) && !isToday(date),
                  'bg-blue-50 text-blue-700 cursor-pointer hover:bg-blue-100': isCurrentMonth(date) && hasEvent(date) && !isToday(date),
            ...
            >
              ...
              <span v-if="hasEvent(date) && isCurrentMonth(date)" class="w-1.5 h-1.5 rounded-full mt-1" :class="isToday(date) ? 'bg-white' : 'bg-blue-500'" />
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          >
              ...
              <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    >
                      <component :is="getEventTypeIcon(event.type)" class="w-3.5 h-3.5" />
                  ...
                  <h3 class="text-xl font-bold text-slate-900">{{ event.name }}</h3>
                  <p class="text-slate-600 mt-1">{{ formatDateRange(event.start_date, event.end_date) }}</p>
                </div>
                <button
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div v-if="event.start_time" class="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p class="font-medium text-slate-900">{{ event.start_time }}</p>
                  </div>
                </div>
                <div v-if="event.cost" class="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p class="font-medium text-slate-900">${{ event.cost.toFixed(2) }}</p>
                  </div>
                </div>
                <div v-if="event.address || event.city" class="flex items-center gap-2">
                  <MapPinIcon class="w-4 h-4 text-slate-400" />
                  <div>
                    <p class="text-xs text-slate-500">Location</p>
                    <p class="font-medium text-slate-900">
                      {{ event.city || event.address }}{{ event.state ? `, ${event.state}` : '' }}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div v-if="event.performance_notes || event.description" class="border-t border-slate-200 px-6 py-4 bg-slate-50">
        ...
        <div v-if="filteredEvents.length === 0 && events.length > 0" class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center">
...
const { events, loading, fetchEvents, deleteEvent: deleteEventAPI } = useEvents()
