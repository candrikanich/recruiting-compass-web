...
export default defineEventHandler(async (event): Promise<BroadcastResponse> => {
  ...
  if (!parsed.success) {
    logger.error("Broadcast validation failed", {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    const { error } = await supabase
      .from("notifications")
      .insert(
        batch as Array<Database["public"]["Tables"]["notifications"]["Insert"]>,
      );
    if (error) {
      logger.error("Failed to insert broadcast batch", error);
      throw createError({
        statusCode: 500,
        statusMessage: "Failed to send broadcast",
      });
    }
  }

  logger.info(
    `Admin broadcast sent: type=${type}, target=${target}, recipients=${userIds.length}`,
  );
