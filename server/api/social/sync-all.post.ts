      ...
      try {
        ...
        if (twitterHandles.length > 0) {
          ...
          for (const post of twitterPosts) {
            ...
            if (!existing) {
              const school = schools?.find((s) => s.twitter_handle === post.author_handle)
              const coach = coaches?.find((c) => c.twitter_handle === post.author_handle)
        ...
        if (instagramHandles.length > 0) {
          ...
          for (const post of instagramPosts) {
            ...
            if (!existing) {
              const school = schools?.find((s) => s.instagram_handle === post.author_handle)
              const coach = coaches?.find((c) => c.instagram_handle === post.author_handle)
