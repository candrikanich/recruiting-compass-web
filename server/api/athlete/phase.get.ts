...
import { calculatePhase, getMilestoneProgress, canAdvancePhase } from '~/utils/phaseCalculation'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const completedTaskIds = athleteTasksData?.map((at: any) => at.task_id).filter(Boolean) as string[]

    // Calculate current phase if not already set
    const currentPhaseValue = (userData as any)?.current_phase
    let currentPhase: Phase = (typeof currentPhaseValue === 'string' ? currentPhaseValue : 'freshman') as Phase
