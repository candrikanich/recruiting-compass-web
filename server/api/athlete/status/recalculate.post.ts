...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const requiredTaskIds = ((requiredTasksData as unknown) as Array<Record<string, any>> || []).map(
      (t) => t.id
    )
    ...
    const completedTaskIds = ((completedTasksData as unknown) as Array<Record<string, any>> || []).map(
      (at) => at.task_id
    )
    ...
    const targetSchools = ((schoolsData as unknown) as Array<Record<string, any>> || []).length
    ...
    if (!interactionsError && interactionsData && interactionsData.length > 0) {
      const interactionsRecords = interactionsData as unknown as Array<Record<string, any>>
    ...
    const academicStandingScore = calculateAcademicStandingScore(
      ...
      (academicRecord?.ncaa_eligibility_status as 'registered' | 'pending' | 'not_started') ??
        'not_started',
      []
    )
