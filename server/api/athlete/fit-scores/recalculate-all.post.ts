...
export default defineEventHandler(async (event): Promise<RecalculationResult> => {
  ...
  try {
    ...
    if (prefError) {
      throw createError({
        statusCode: 400,
        statusMessage: "Player details not configured. Add athlete profile data first.",
      });
    }
    ...
    if (!playerDetails) {
      throw createError({
        statusCode: 400,
        statusMessage: "Player details not found. Add athlete profile data first.",
      });
    }
    ...
    for (const school of schools) {
      try {
        ...
        console.error(`Failed to calculate fit score for school ${school.id}:`, err);
