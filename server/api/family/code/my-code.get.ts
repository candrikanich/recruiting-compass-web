...
export default defineEventHandler(async (event) => {
  const logger = useLogger(event, "family/code");
  try {
    ...
    if (userRole === "player") {
      ...
      logger.info("Fetched family code for player");
    ...
    logger.info("Fetched family codes for parent");
    ...
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to fetch family code",
    });
  }
});
