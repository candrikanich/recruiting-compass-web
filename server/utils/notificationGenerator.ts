...
async function shouldSendEmail(supabase: SupabaseClient, userId: string, priority: string): Promise<boolean> {
  try {
    ...
    if (prefs.notification_settings.emailOnlyHighPriority && priority !== 'high') return false
    return true
  } catch {
    return false
  }
...
const TEMPLATES: Record<string, NotificationTemplate> = {
  ...
  offer_deadline_7: {
    ...
    message: (schoolName: string) => `Deadline for your ${schoolName} offer is in one week.`,
  },
  offer_deadline_3: {
    title: (schoolName: string) => `Urgent: Offer deadline in 3 days - ${schoolName}`,
    message: (schoolName: string) => `Your offer from ${schoolName} expires in 3 days!`,
  },
  offer_deadline_1: {
    title: (schoolName: string) => `FINAL: Offer expires tomorrow - ${schoolName}`,
    message: (schoolName: string) => `Last day to respond to ${schoolName} offer!`,
  },
  rec_deadline_14: {
    title: (name: string) => `Recommendation due in 14 days from ${name}`,
    message: (name: string) => `Rec letter from ${name} is due in 2 weeks. Follow up if needed.`,
  },
  rec_deadline_7: {
    title: (name: string) => `Recommendation due in 7 days from ${name}`,
    message: (name: string) => `Rec letter deadline from ${name} is one week away.`,
  },
  event_upcoming_7: {
    title: (eventName: string) => `Event in 7 days: ${eventName}`,
    message: (eventName: string) => `Don't forget: ${eventName} is coming up in one week. Prepare accordingly.`,
  },
  event_upcoming_1: {
    title: (eventName: string) => `Event tomorrow: ${eventName}`,
    message: (eventName: string) => `${eventName} is tomorrow. Final preparations today!`,
  },
  coach_followup: {
 ...
 */
export async function generateOfferNotifications(userId: string, supabase: SupabaseClient): Promise<NotificationGenerationResult> {
  try {
    ...
    for (const offer of offers) {
      ...
      for (const days of leadTimes) {
        if (daysUntil === days) {
          ...
          if (!existing) {
            const template = TEMPLATES[`offer_deadline_${days}` as keyof typeof TEMPLATES]
            await supabase
              .from('notifications')
              .insert([
                {
 ...
 */
export async function generateEventNotifications(userId: string, supabase: SupabaseClient): Promise<NotificationGenerationResult> {
  try {
    ...
    for (const event of events) {
      ...
      for (const days of leadTimes) {
        if (daysUntil === days) {
          ...
          if (!existing) {
            const templateKey = `event_upcoming_${days}` as keyof typeof TEMPLATES
            const template = TEMPLATES[templateKey]
            await supabase
              .from('notifications')
              .insert([
...
export async function generateCoachFollowupNotifications(
  ...
  try {
    ...
    for (const coach of coaches) {
      ...
      const daysSince = Math.ceil((now.getTime() - lastContact.getTime()) / (1000 * 60 * 60 * 24))
 ...
 */
export async function generateDailyDigest(userId: string, supabase: SupabaseClient): Promise<NotificationGenerationResult> {
  try {
