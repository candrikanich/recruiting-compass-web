...
**Account pool (#970):** `TEST_EMAIL` is account 0 of a pool; siblings are auto-derived as `k6-load-test-1@example.com`, `-2@`, etc. (index inserted before the `@`). Pool size defaults to 5 and is configurable via `TEST_EMAIL_POOL_SIZE`. Each VU picks an account by `__VU % poolSize`, spreading load across distinct `server/middleware/rate-limit.ts` keys. Runs 2 and 3 (2026-09-22) shared one token across all VUs and hit the 60/min ceiling regardless of endpoint; a first pooled attempt at 5 tokens made no difference either, because the rate limiter keys off the `sb-access-token` **cookie**, not the `Authorization` header this script sends — so all 5 accounts still collapsed onto one IP-based bucket until the request also carries that cookie (see `default()` in `api-load.js`). Full history in `k6/findings.md`. The 5 accounts already exist on QA (created via the Supabase Admin API, `email_confirm: true`, no password) — if you change `TEST_EMAIL` to a different base or raise the pool size, create the matching sibling accounts the same way first.
