...
const COMPLETION_MARKER_RE =
  /^##\s+Status:\s*(Complete|Done|Merged|Implemented)\s*$/im;
...
export function isAutoDelete(relPath, base, mtime) {
  ...
  if (
    relPath.startsWith("archived-docs/") &&
    Date.now() - mtime > THIRTY_DAYS_MS + 1000
  )
    return true;
...
export function detectPairs(items) {
  ...
  const toSlug = (base) =>
    base.replace(/-design\.md$/i, ".md").replace(/-plan\.md$/i, ".md");
...
export async function scan(repoRoot, repoName) {
  ...
  console.log(
    `  Compress    : ${compressRaw.length} files → ${compress.length} history entries`,
  );
