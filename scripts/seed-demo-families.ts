...
interface DemoSchool {
  ...
  coach: {
    first_name: string;
    last_name: string;
    role: "head" | "recruiting" | "assistant";
  };
  interactions: Array<{
    type: string;
        ...
        interactions: [
          {
            ...
            subject: "Loved your highlight reel",
            content:
              "Coach Reyes reached out after the fall showcase — wants to stay in touch.",
            daysAgo: 4,
          },
          {
            type: "camp",
            direction: "outbound",
            sentiment: "positive",
            subject: "Registered for ID camp",
            content: "Signed up for the December ID camp on campus.",
            daysAgo: 12,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            sentiment: "positive",
            subject: "Intro call",
            content: "15-min call with the head coach about academics and fit.",
            daysAgo: 9,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            direction: "outbound",
            sentiment: "neutral",
            subject: "Intro + transcript",
            content: "Sent intro email with transcript and season schedule.",
            daysAgo: 20,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            direction: "outbound",
            sentiment: "positive",
            subject: "Saw them at regionals",
            content:
              "Watched Davidson staff at the regional showcase; strong academic match.",
            daysAgo: 27,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            direction: "inbound",
            sentiment: "very_positive",
            subject: "Official visit invite",
            content:
              "Invited for an official visit the weekend after Thanksgiving.",
            daysAgo: 3,
          },
          {
            type: "phone_call",
            direction: "inbound",
            sentiment: "very_positive",
            subject: "Scholarship conversation",
            content: "Head coach discussed a partial scholarship offer.",
            daysAgo: 6,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            content: "Toured campus and met two assistants after a home game.",
            daysAgo: 11,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            subject: "Twitter DM",
            content:
              "Assistant slid into DMs after a walk-off win at the summer tournament.",
            daysAgo: 15,
          },
        ],
      },
        ...
        interactions: [
          {
            ...
            sentiment: "neutral",
            subject: "Intro email",
            content: "Sent intro with stats and film link.",
            daysAgo: 22,
          },
        ],
      },
...
async function resetPlayerContent(
  supabase: Supabase,
  playerId: string,
): Promise<void> {
...
async function ensureFamilyUnit(
  ...
  const { data: membership } = await supabase
    ...
    .maybeSingle();
  if (membership)
    return (membership as { family_unit_id: string }).family_unit_id;
  ...
  const { data: unit, error } = await supabase
    ...
    .single();
  if (error || !unit)
    throw new Error(`family_units insert failed: ${error?.message}`);
...
async function ensureMember(
  ...
  if (error)
    throw new Error(`family_members insert (${role}) failed: ${error.message}`);
}
...
async function seedSchools(
  ...
  for (const school of schools) {
    ...
    if (schoolErr || !row)
      throw new Error(
        `schools insert (${school.name}) failed: ${schoolErr?.message}`,
      );
    ...
    const { data: coachRow, error: coachErr } = await supabase
      ...
      .insert({
        ...
        user_id: playerId,
        family_unit_id: familyUnitId,
        first_name: school.coach.first_name,
        last_name: school.coach.last_name,
    ...
    if (coachErr)
      throw new Error(
        `coaches insert (${school.name}) failed: ${coachErr.message}`,
      );
    ...
    const interactionRows = school.interactions.map((i) => ({
      ...
      subject: i.subject,
      content: i.content,
      occurred_at: daysAgoISO(i.daysAgo),
    }));
    ...
    if (intErr)
      throw new Error(
        `interactions insert (${school.name}) failed: ${intErr.message}`,
      );
  }
}
...
async function seedCompletedTasks(
  ...
  const { error } = await supabase
    .from("athlete_task")
    .upsert(rows, { onConflict: "athlete_id,task_id" });
...
async function seedFamily(
  ...
  const playerUser = await createOneOffTestUser({
    email: family.player.email,
    password: SHARED_PASSWORD,
    displayName: family.player.displayName,
    role: "player",
  });
  ...
  console.log(
    `   👤 player ${family.player.email} / parent ${family.parent.email}`,
  );
  ...
  if (playerUpdErr)
    throw new Error(`users upsert (player) failed: ${playerUpdErr.message}`);
  ...
  if (parentUpdErr)
    throw new Error(`users upsert (parent) failed: ${parentUpdErr.message}`);

  const familyUnitId = await ensureFamilyUnit(
    supabase,
    playerId,
    family.familyName,
  );
  ...
  const taskCount = await seedCompletedTasks(
    supabase,
    playerId,
    family.player.graduationYear,
  );
  console.log(
    `   🎓 ${family.schools.length} schools + coaches + interactions, ${taskCount} tasks completed`,
  );
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `Seeding ${FAMILIES.length} demo families → ${process.env.NUXT_PUBLIC_SUPABASE_URL}`,
  );
