...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error(
    "  4. Copy the key and add to .env.local: SUPABASE_SERVICE_ROLE_KEY=...",
  );
  process.exit(1);
}
...
async function applyMigration(filename) {
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        `   Note: Use Supabase Console to manually execute this migration`,
      );
      return false;
    }
...
async function main() {
  ...
  if (failed > 0) {
    ...
    console.log(
      "5. Copy content from web/server/migrations/004_create_parent_view_helpers.sql",
    );
